...
test('快捷键设置模块提供两个默认值，并可持久化到用户配置文件', () => {
  assert.ok(fs.existsSync(modulePath), 'shortcut-settings.js should exist');
  const { DEFAULT_SHORTCUT, DEFAULT_JUMP_SHORTCUT, loadShortcutSettings, saveShortcutSettings } = require('./shortcut-settings');
...
test('旧版单快捷键配置会补齐最近完成任务快捷键默认值', () => {
  const { DEFAULT_JUMP_SHORTCUT, loadShortcutSettings } = require('./shortcut-settings');
