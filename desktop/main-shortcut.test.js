...
test('Electron 主进程接入云端授权 IPC 和 Windows 安全存储', () => {
  assert.match(source, /safeStorage/);
  assert.match(source, /createSecureStore/);
  assert.match(source, /cloud:status/);
  assert.match(source, /cloud:login/);
  assert.match(source, /cloud:register/);
  assert.match(source, /cloud:forgot-password/);
  assert.match(source, /cloud:redeem/);
  assert.match(source, /cloud:acquire/);
  assert.match(source, /cloud:release/);
  assert.match(source, /releaseActiveLease/);
});
