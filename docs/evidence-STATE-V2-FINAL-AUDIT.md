...
- Fresh `main` NSIS/unpacked package `dist-state-engine-v2-20260913/`: electron-builder and package verifier passed; fully isolated packaged Electron smoke passed against `/api/ready`, `/api/state` and `/api/health` (see `STATE-V2-30`).
...
- The follow-up that isolates backend data/config/source paths is recorded in `docs/evidence-STATE-V2-30.md`; the earlier non-isolated run is superseded.
