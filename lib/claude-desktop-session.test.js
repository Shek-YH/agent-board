...
test('CLI-only 会话使用只带 session 的 resume 深链，避免 Windows handler 误判 cwd', () => {
  const root = fixtureRoot();
  try {
    ...
    assert.equal(
      target.deepLink,
      'claude://resume?session=58f71b61-20f3-454e-bc48-fe3d665b9e37',
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test('已经导入的 CLI 会话继续使用 resume 并按 local_<cliSessionId> 去重', () => {
  ...
  try {
    ...
    assert.equal(target.deepLink, `claude://resume?session=${CLI_ID}`);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
