...
test('uses the verified request title when the session store candidate has no title', () => {
  const result = enrichVerifiedTarget(
    { agent: 'codex', project: 'C:/repo', sessionRef: 'codex:thread-1', title: 'Visible Codex title' },
    {
      status: 'resolved',
      target: { sessionRef: 'codex:thread-1', role: 'main' },
      candidates: [{
        sessionRef: 'codex:thread-1', agent: 'codex', project: 'C:/repo',
        role: 'main', controlEligibility: 'eligible',
      }],
    },
  );

  assert.equal(result.title, 'Visible Codex title');
});
