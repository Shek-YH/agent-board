...
function ingestSession(
  ...
  const finishReasonKey = `${offsetKey}:finish-reason`;
  const stored = store.stmts.getMeta.get(offsetKey)?.v;
  const lastMessageId = stored == null ? -1 : Number(stored);
  const storedFinishReason = store.stmts.getMeta.get(finishReasonKey)?.v;
  let rows;
  let contextRows;
  let latestMessage;
  try {
    ...
    const candidateRows = db.prepare(queries.rows).all(sid, lastMessageId, lastMessageId);
    // The query intentionally includes the last assistant row so a finish_reason
    // transition from tool_calls to stop can be observed. Once that reason has
    // been seen, do not replay the same terminal row on every poll.
    rows = candidateRows.filter((row) => {
      const id = Number(row.id);
      const terminalLastRow = id === lastMessageId
        && row.role === 'assistant'
        && row.finish_reason
        && String(row.finish_reason) !== 'tool_calls';
      return !terminalLastRow || String(row.finish_reason) !== String(storedFinishReason || '');
    });
  ...
  const latestId = Number(latestMessage && latestMessage.id);
  const latestFinishReason = String((latestMessage && latestMessage.finish_reason) || '');
  if (Number.isFinite(latestId) && latestId === maxMessageId && latestFinishReason !== String(storedFinishReason || '')) {
    store.stmts.setMeta.run(finishReasonKey, latestFinishReason);
  }
  return count;
}
