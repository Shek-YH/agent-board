...
function parseSessionRows(session, rows, contextRows) {
...
function ingestSession(db, session, store, latestMessageStmt, queries = messageQueries(db)) {
  const sid = String(session.id || '');
  ...
  for (const event of parseSessionRows(session, rows, contextRows)) {
    store.ingest(event);
...
function scanFromDb(store, dbPath = DB) {
  ...
  try {
    ...
    for (const session of sessions) count += ingestSession(db, session, store, latestMessageStmt, queries);
  } catch (e) {
...
module.exports = {
  ...
  messageQueries,
  scanDb(dbPath, store) { return scanFromDb(store, dbPath); },
