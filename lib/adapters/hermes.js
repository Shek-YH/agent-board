...
// 数据源：%LOCALAPPDATA%\hermes\state.db（SQLite + WAL）
...
const ID = 'hermes';
const ROOT = path.join(os.homedir(), 'AppData', 'Local', 'hermes');
...
function ingestSession(db, session, store, latestMessageStmt) {
  ...
  try {
    latestMessage = latestMessageStmt
      ...
      : db.prepare('SELECT id, role, timestamp, finish_reason FROM messages WHERE session_id = ? ORDER BY id DESC LIMIT 1').get(sid);
    rows = db.prepare(
...
function scanFromDb(store) {
  ...
  try {
    ...
    const latestMessageStmt = db.prepare(
      'SELECT id, role, timestamp, finish_reason FROM messages WHERE session_id = ? ORDER BY id DESC LIMIT 1'
    );
