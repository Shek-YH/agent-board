...
test('WorkBuddy completed assistant message emits an immediate child turn end', () => {
  const events = workbuddy.parseLines([{
    type: 'message', sessionId: 'child-completed', role: 'assistant', status: 'completed',
    timestamp: 2000, content: '743629',
  }], childPath);
  assert.equal(events.length, 2);
  assert.equal(events[0].kind, 'message');
  assert.equal(events[1].kind, 'turn_end');
  assert.equal(events[1].sessionId, 'child-completed');
  assert.equal(events[1].parentSessionId, 'parent-from-path');
});
