...
const ID = 'marvis';
const ROOT = SOURCE_PATHS.marvis;
...
function ingestConversation(db, conv, store) {
  ...
  // 注意：必须放在消息入库【之后】——store 的完成候选在收到「10 分钟内的新真实消息」时会无条件
  // 撤销（lib/store.js 的 ingest 取消点）。若先发 turn_end 再 ingest 本会话残余消息，候选会被
  // 随即到来的消息顶掉且 turnEndSeen 已去重，导致该终态永不广播。
  if (TERMINAL_STATUSES.has(conv.status)) {
    const signalTs = parseTs(conv.updated_at);
    // cancelled 是被用户/系统打断（如手动停止）而非真正完成：沿用 store 的约定（turnStatus
    // interrupted/cancelled → suppressBroadcast），只翻卡片为已完成、不弹完成通知；completed/failed
    // 视为会话已结束，正常广播。
    const cancelled = conv.status === 'cancelled';
    const turnEndBase = {
      agent: ID, ts: signalTs, kind: 'turn_end',
      ...(cancelled ? { turnStatus: 'cancelled' } : {}),
    };
    store.ingest({
      ...turnEndBase, sourceId: `turnend:${convId}:${conv.updated_at}`,
      sessionId: convId, ...mainTopology,
    });
    for (const topology of childTopologies) {
      const { title, ...eventTopology } = topology;
      store.ingest({
        ...turnEndBase, sourceId: `turnend:${topology.sessionId}:${conv.updated_at}`,
        sessionId: topology.sessionId, project, ...eventTopology,
      });
    }
  }
  return n;
...
function scanFromDb(dbPath, store, { cutoff = 0 } = {}) {
  let count = 0;
  let db;
  ...
  try {
    ...
    for (const c of convs) {
      const activityAt = Math.max(parseTs(c.updated_at), parseTs(c.created_at));
      if (cutoff > 0 && activityAt > 0 && activityAt < cutoff) continue;
      count += ingestConversation(db, c, store);
    }
  } catch (e) {
...
module.exports = {
  ...
  scanAll(store, options = {}) {
    let count = 0;
    for (const db of listUserDbs()) count += scanFromDb(db, store, options);
    return count;
  },
