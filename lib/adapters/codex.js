...
function completionHoldMsForHistory(events, signalTs) {
  ...
  return Math.max(MULTI_TURN_DEFAULT_HOLD_MS, Math.max(...rapidGaps) + COMPLETION_BUFFER_MS);
}
...
module.exports = {
  ID, ROOT, isSessionFile, parseLines, completionHoldMsForHistory, fileToSessionId: sessionIdFromPath, detect, reconcileRecentCompletions,
  scanAll(store) {
