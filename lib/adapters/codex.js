  ...
  lines.forEach((line, idx) => {
    try {
      ...
      if (t === 'event_msg' && pl.type === 'task_started' && ts) {
        ...
        out.push({
          agent: ID, sourceId: `${sid}:${turnId || ts}:turnstart`, sessionId: sid, ts,
          kind: 'turn_start', turnId, turnStatus: 'inProgress',
        });
        return;
      }
      if (t === 'event_msg' && pl.type === 'turn_aborted' && ts) {
        const turnId = pl.turn_id ? String(pl.turn_id) : null;
        out.push({
          agent: ID, sourceId: `${sid}:${turnId || ts}:turnabort`, sessionId: sid, ts,
          kind: 'turn_end', turnId, turnStatus: 'interrupted',
        });
        return;
      }
      ...
      if (t === 'event_msg' && pl.type === 'task_complete' && ts) {
        ...
        out.push({
          agent: ID, sourceId: `${sid}:${ts}:turnend`, sessionId: sid, ts,
          kind: 'turn_end', turnId, turnStatus, completionHoldMs: turnStatus === 'completed' ? completionHoldMs : 0,
        });
        return;
      }
