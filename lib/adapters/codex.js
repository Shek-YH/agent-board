...
const SESSION_INDEX = SOURCE_PATHS.codexSessionIndex;
...
const completionHistory = createCompletionHistory({ readAll });
...
function applySessionIndexTitles(store, titles) {
  ...
  for (const [sessionId, title] of titles) {
    ...
    const exactRef = `${ID}:${sessionId}`;
    const exactSession = typeof store.getSession === 'function' ? store.getSession(exactRef) : null;
    if (!ref && exactSession && String(exactSession.id || '').trim() === exactRef) ref = exactRef;
    if (!ref) {
      const candidate = store.resolveAgentRef(ID, sessionId);
      const candidateSessionId = typeof candidate === 'string' && candidate.startsWith(`${ID}:`)
        ? candidate.slice(ID.length + 1)
        : '';
      const matchesSession = Boolean(candidateSessionId) && Boolean(sessionId) && (
      ...
      if (matchesSession) {
        ref = candidate;
        sessionIndexRefs.set(sessionId, ref);
      }
    }
    if (!ref) continue;
...
// rollout 文件名带有写入时间，但 session_meta.payload.id 才是 Codex thread ID。
// 文件名仍作为旧版本/损坏元数据的回退，并统一取末尾 UUID，避免把时间戳误当作 ID。
function sessionIdFromPath(p) {
  const raw = path.basename(p).replace(/\.jsonl$/, '').replace(/^rollout-/, '');
  const match = raw.match(UUID_SUFFIX);
...
function sessionMetaLine(lines) {
  return lines.find((line) => line && line.type === 'session_meta' && line.payload && typeof line.payload === 'object') || null;
}

function sessionIdFromLines(lines, filePath) {
  const payload = sessionMetaLine(lines)?.payload || {};
  const id = String(payload.id || '').trim();
  return id || sessionIdFromPath(filePath);
}

function readSessionMetaLine(filePath) {
  let fd = null;
  try {
...
function topologyForSessionMeta(payload) {
  const pl = objectValue(payload);
  const source = objectValue(pl.source);
  const subagent = objectValue(source.subagent);
  const spawn = objectValue(subagent.thread_spawn);
  const parentId = String(
    spawn.parent_thread_id || pl.parent_thread_id || subagent.parent_thread_id || '',
  ).trim();
  const isSubagent = Boolean(source.subagent) || pl.thread_source === 'subagent' || Boolean(parentId);
  if (!isSubagent) {
    return {
      sessionRole: 'main', topologySource: 'structural', topologyConfidence: 1,
      childDetection: 'verified', controlEligibility: 'eligible',
    };
  }
  const rootId = String(spawn.root_thread_id || pl.root_thread_id || parentId).trim();
  const depthValue = Number(spawn.depth ?? pl.depth);
  return {
    sessionRole: 'child',
    parentSessionId: parentId || undefined,
    rootSessionId: rootId || parentId || undefined,
    topologySource: spawn.parent_thread_id ? 'explicit' : 'structural',
    topologyConfidence: spawn.parent_thread_id ? 1 : 0.9,
    childDetection: 'verified',
    controlEligibility: 'blocked',
    topologyDepth: Number.isFinite(depthValue) ? depthValue : undefined,
    agentNickname: String(spawn.agent_nickname || pl.agent_nickname || '').trim() || undefined,
    agentRole: String(spawn.agent_role || pl.agent_role || '').trim() || undefined,
  };
}
...
function completionHoldMsForSession(filePath, signalTs, fallbackEvents) {
  return completionHistory.holdMs(filePath, signalTs, fallbackEvents);
}

function parseLines(lines, filePath) {
  const out = [];
  const sid = sessionIdFromLines(lines, filePath);
  const metaLine = sessionMetaLine(lines);
  const topology = metaLine ? topologyForSessionMeta(metaLine.payload) : {};
  const event = (fields) => ({ agent: ID, sessionId: sid, ...topology, ...fields });
  lines.forEach((line, idx) => {
    try {
      ...
      if (t === 'session_meta' && pl.cwd) {
        out.push(event({ sourceId: `${sid}:meta`, ts: ts || 0, role: 'system', kind: 'heartbeat', text: '', project: pl.cwd || '' }));
        return;
      }
      // 任务回合生命周期：保留 turn_id，供状态归并层区分同一 session 内的多个回合。
      if (t === 'event_msg' && pl.type === 'task_started' && ts) {
        const turnId = pl.turn_id ? String(pl.turn_id) : null;
        out.push(event({ sourceId: `${sid}:${turnId || ts}:turnstart`, ts, kind: 'turn_start', turnId, turnStatus: 'inProgress' }));
        return;
      }
      if (t === 'event_msg' && pl.type === 'turn_aborted' && ts) {
        const turnId = pl.turn_id ? String(pl.turn_id) : null;
        out.push(event({ sourceId: `${sid}:${turnId || ts}:turnabort`, ts, kind: 'turn_end', turnId, turnStatus: 'interrupted' }));
        return;
      }
      ...
      if (t === 'event_msg' && pl.type === 'task_complete' && ts) {
        ...
        const turnId = pl.turn_id ? String(pl.turn_id) : null;
        const turnStatus = ['completed', 'interrupted', 'failed'].includes(pl.status) ? pl.status : 'completed';
        out.push(event({ sourceId: `${sid}:${ts}:turnend`, ts, kind: 'turn_end', turnId, turnStatus, completionHoldMs: turnStatus === 'completed' ? completionHoldMs : 0 }));
        return;
      }
      ...
      if (t === 'user_message') {
        ...
        if (text) out.push(event({ sourceId: `${sid}:${ts}:${idx}:u`, ts, role: 'user', kind: 'message', text, project: pl.cwd || '' }));
        return;
      }
      if (t === 'agent_message') {
        const text = extractPayloadText(pl);
        if (text) out.push(event({ sourceId: `${sid}:${ts}:${idx}:a`, ts, role: 'assistant', kind: 'message', text, project: pl.cwd || '' }));
        return;
      }
      if (t === 'summary') {
        const text = extractPayloadText(pl);
        if (text) out.push(event({ sourceId: `${sid}:${ts}:${idx}:s`, ts, role: 'assistant', kind: 'summary', text, project: pl.cwd || '' }));
        return;
      }
      ...
      if (t === 'response_item' && pl.type === 'message') {
        ...
        out.push(event({ sourceId: `${sid}:${pl.id || ts}:${idx}`, ts, role, kind: 'message', text, project: pl.cwd || '' }));
      }
    } catch { /* skip */ }
  });
  return out;
...
function updateCodexActivity(store, filePath, lines, msgs) {
  ...
  if (lines.some(isDefinitiveContinuationLine)) {
    store.confirmCodexContinuation(`${ID}:${sessionIdFromLines(lines, filePath)}`, sourceTs, info);
  } else {
    store.noteCodexActivity(`${ID}:${sessionIdFromLines(lines, filePath)}`, sourceTs, info);
  }
}
...
const detect = {
  ...
  // Codex CLI 与 ChatGPT/Codex Desktop 是两个变体；桌面端只用于安装识别和启动路径，
  // 不拿它执行 codex --version，也不写入 CLI tool-path 覆盖。
  desktopProbe: {
    probe: {
      kind: 'path',
      executable: true,
      ...
      linux: [],
      registryHints: {
        windowsDisplayNames: ['ChatGPT', 'Codex'],
        windowsPublisher: 'OpenAI',
      },
    },
  },
  install: {
    methods: [
      { kind: 'download', url: 'https://github.com/openai/codex/releases/latest' },
    ],
    warning: '请在 OpenAI Codex 官方发布页完成安装；Agent Board 不会执行第三方安装命令',
  },
  network: {
...
module.exports = {
  ID, ROOT, SESSION_INDEX, isSessionFile, parseSessionIndexLines, applySessionIndexTitles, syncSessionTitles, parseLines, completionHoldMsForHistory, isDefinitiveContinuationLine, fileToSessionId: sessionIdFromPath, readSessionMetaLine, detect, reconcileRecentCompletions,
  scanAll(store) {
    ...
    for (const f of files) {
      ...
      if (lines.length) {
        ...
        // 存量数据的 offset 可能已经在文件末尾；仍需重读 session_meta，
        // 让新版本拓扑识别可以把旧的“主会话”记录升级为子代理。
        const metaLine = readSessionMetaLine(f);
        if (metaLine) {
          for (const m of parseLines([metaLine], f)) { store.ingest(m); count++; }
        }
      }
