...
function claudeFileTopology(filePath, line = {}) {
  const match = String(filePath || '').match(/[\\/]subagents[\\/]agent-([^\\/]+)\.jsonl$/i);
  if (!match) {
    return {
  ...
  const parentSessionId = String(line.sessionId || '').trim();
  const agentId = String(line.agentId || match[1]).trim();
