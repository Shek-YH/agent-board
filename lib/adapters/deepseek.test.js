...
test('DeepSeek topology is included on title and turn-end events', () => {
  ...
  assert.equal(turnEnd.sessionRole, 'child');
  assert.equal(turnEnd.parentSessionId, 'parent-events');
  assert.equal(title.sessionRole, 'child');
  assert.equal(title.parentSessionId, 'parent-events');
});

test('DeepSeek message and turn-end source IDs are scoped to their session', () => {
  const parse = (sessionId) => deepseek.parseLines([
    { type: 'session', id: sessionId },
    { type: 'user/message', seq: 1, time: 1000, data: { source: { kind: 'user' }, content: 'hello' } },
    { type: 'assistant/message', seq: 2, time: 1001, data: { message: { content: 'world' } } },
    { type: 'turn/end', seq: 3, time: 1002, data: { reason: { kind: 'completed' } } },
  ]);
  const first = parse('session-a');
  const second = parse('session-b');
  for (const kind of ['message', 'turn_end']) {
    const firstEvent = first.find((event) => event.kind === kind);
    const secondEvent = second.find((event) => event.kind === kind);
    assert.notEqual(firstEvent.sourceId, secondEvent.sourceId, `${kind} source IDs must not collide`);
    assert.match(firstEvent.sourceId, /^session-a:/);
    assert.match(secondEvent.sourceId, /^session-b:/);
  }
});

test('DeepSeek mtime replay cache uses the session-identity migration version', () => {
  const source = fs.readFileSync(path.join(__dirname, 'deepseek.js'), 'utf8');
  assert.match(source, /mtime:v4/);
});
