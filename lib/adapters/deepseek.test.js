...
test('DeepSeek descriptor is child evidence only when parentSession is present', () => {
  const child = deepseek.parseLines([
    ...
    { type: 'subagent/descriptor', data: { id: 'descriptor-2', name: 'Explorer' } },
  ...
  const descriptorWithoutParent = deepseek.parseLines([
    ...
    { type: 'subagent/descriptor', data: { id: 'descriptor-3', name: 'Explorer' } },
...
test('DeepSeek malformed or empty descriptors do not create child sessions', () => {
  for (const descriptor of [
    { type: 'subagent/descriptor' },
    { type: 'subagent/descriptor', data: {} },
    { type: 'subagent/descriptor', data: { id: '   ', name: 'Explorer' } },
  ]) {
    const event = deepseek.parseLines([
      { type: 'session', id: 'malformed-descriptor', parentSession: 'parent-3' },
      descriptor,
      { type: 'assistant/message', seq: 1, time: 1000, data: { message: { content: 'main' } } },
    ]).find((item) => item.kind === 'message');
    assert.equal(event.sessionRole, 'main');
    assert.equal(event.controlEligibility, 'eligible');
  }
});
