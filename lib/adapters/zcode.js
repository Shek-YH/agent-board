...
const ID = 'zcode';
const ROOT = SOURCE_PATHS.zcode;
const DB = path.join(ROOT, 'db.sqlite');

function topologyForSession(sess = {}) {
  const taskType = String(sess.task_type || '').trim();
  ...
  if (taskType === 'subagent_child') {
    return {
      sessionRole: 'child', topologySource: 'explicit', topologyConfidence: 1,
      childDetection: 'verified', controlEligibility: 'blocked',
    };
  }
  return {
    sessionRole: 'unknown', topologySource: 'explicit', topologyConfidence: 0,
    childDetection: 'verified', controlEligibility: 'unknown',
  };
}
...
function openDb(dbPath = DB) {
  ...
  try { return new DatabaseSync(dbPath, { readOnly: true }); } catch (e) { console.error(`[${ID}] open db failed:`, e.message); return null; }
}
...
function ingestSession(db, sess, store) {
  ...
  const topology = topologyForSession(sess);
  ...
  // 还会继续调用工具，非终态）。part 有独立的时间游标，用独立 offset 增量扫描——
  ...
  // message 早已判定"非新增"时 step-finish 会被永久漏读。不能用 part.sequence：它在
  // 不同 message 内重新计数，不是跨扫描的全局递增游标。
  const partOffsetKey = `offset:${ID}:part:${sid}`;
  const partStored = store.stmts.getMeta.get(partOffsetKey)?.v;
  // 旧版本存的是 sequence（通常只有个位数）；切换到毫秒时间游标后会自然触发一次重读，
  // 让已有数据中的 step-finish:stop 得到补采，不需要手工清理 data.json。
  const partLastTime = partStored == null ? -1 : Number(partStored);
  try {
    const newParts = db.prepare(
      'SELECT id, time_created, data, sequence FROM part WHERE session_id = ? AND time_created > ? ORDER BY time_created, id'
    ).all(sid, partLastTime);
    let maxPartTime = partLastTime;
    for (const p of newParts) {
      const partTime = Number(p.time_created);
      if (Number.isFinite(partTime) && partTime > maxPartTime) maxPartTime = partTime;
      try {
        const d = JSON.parse(p.data);
        if (d.type === 'step-finish' && d.reason === 'stop') {
          store.ingest({
            agent: ID, sourceId: `turnend:${p.id}`, sessionId: sid,
            ts: Number(p.time_created) || 0, kind: 'turn_end',
            // step-finish:stop = 一轮回复正常结束（zcode 数据源没有 interrupted 标记；
            // 不在 store 稳定窗表，turn_end 只翻卡片、不广播弹窗）。
            turnStatus: 'completed', ...topology,
          });
        }
      } catch { /* skip */ }
    }
    if (maxPartTime !== partLastTime) store.stmts.setMeta.run(partOffsetKey, String(maxPartTime));
  } catch (e) {
...
function scanFromDb(db, store, { cutoff = 0 } = {}) {
  let total = 0;
  try {
    const sessions = db.prepare('SELECT id, title, directory, task_type FROM session').all();
    const latestMessage = db.prepare('SELECT MAX(time_created) AS latest FROM message WHERE session_id = ?');
    for (const s of sessions) {
      const activityAt = Number(latestMessage.get(s.id)?.latest) || 0;
...
const detect = {
  ...
  install: {
    ...
    warning: '请在 ZCode 官方页面完成安装；Agent Board 不会执行第三方安装命令',
  },
...
module.exports = {
  ID, ROOT, detect, topologyForSession,
  scanDb(dbPath, store, options = {}) {
    const db = openDb(dbPath);
    if (!db) return 0;
    try { return scanFromDb(db, store, options); } finally { try { db.close(); } catch {} }
  },
  scanAll(store, options = {}) {
    const db = openDb();
    if (!db) return 0;
    try { return scanFromDb(db, store, options); } finally { try { db.close(); } catch {} }
  },
