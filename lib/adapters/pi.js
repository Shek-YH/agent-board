...
function readSessionHeaderId(file) {
  ...
  try {
    ...
    return header && header.type === 'session' && typeof header.id === 'string' && header.id
      ? header.id
      : null;
  } catch {
    return null;
...
function fileToSessionId(file) {
  ...
  if (headerId) {
    sessionIdsByFile.set(file, headerId);
    return headerId;
  }
...
module.exports = {
  ID, ROOT, isSessionFile, parseLines, fileToSessionId, resolveSessionId, detect,
  scanAll(store) {
