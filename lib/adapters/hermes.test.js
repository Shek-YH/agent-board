...
test('does not replay the terminal assistant row on every Hermes poll', () => {
  const fixture = tempHermesDb(
    `CREATE TABLE sessions (
       id TEXT PRIMARY KEY, title TEXT, cwd TEXT, profile_name TEXT,
       ended_at REAL, end_reason TEXT, last_activity_at REAL, started_at REAL
     ...
     INSERT INTO sessions (id, title, cwd, started_at)
       VALUES ('terminal-session', 'Terminal', 'C:/terminal', 1787578000);`,
    [
      { id: 1, session_id: 'terminal-session', role: 'user', content: '完成任务', timestamp: 1787578000, finish_reason: null },
      { id: 2, session_id: 'terminal-session', role: 'assistant', content: '已完成', timestamp: 1787578001, finish_reason: 'stop' },
    ],
  );
  const store = testStore();
  try {
    assert.equal(hermes.scanDb(fixture.dbPath, store), 2);
    assert.equal(hermes.scanDb(fixture.dbPath, store), 0);
  } finally {
    fs.rmSync(fixture.dir, { recursive: true, force: true });
  }
});

test('replays the last assistant row once when finish_reason changes', () => {
  const fixture = tempHermesDb(
    `CREATE TABLE sessions (
       id TEXT PRIMARY KEY, title TEXT, cwd TEXT, profile_name TEXT,
       ended_at REAL, end_reason TEXT, last_activity_at REAL, started_at REAL
     ...
     INSERT INTO sessions (id, title, cwd, started_at)
       VALUES ('transition-session', 'Transition', 'C:/transition', 1787578000);`,
    [{ id: 1, session_id: 'transition-session', role: 'assistant', content: '继续工作', timestamp: 1787578000, finish_reason: 'tool_calls' }],
  );
  const store = testStore();
  try {
    assert.equal(hermes.scanDb(fixture.dbPath, store), 1);
    const db = new DatabaseSync(fixture.dbPath);
    db.prepare('UPDATE messages SET finish_reason = ? WHERE id = ?').run('stop', 1);
    db.close();
    assert.equal(hermes.scanDb(fixture.dbPath, store), 1);
    assert.equal(hermes.scanDb(fixture.dbPath, store), 0);
  } finally {
...
// 端到端闭环：真实 store（隔离 AB_DATA_DIR）+ hermes adapter。ended_at 主会话经
// 通用完成出口广播一次；原生 child（即便 ended_at + finish_reason=stop 都具备）不广播。
test('Hermes 结束的主会话经通用完成出口广播，原生 child 不广播', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-board-hermes-loop-'));
  const dbPath = path.join(dir, 'state.db');
  const db = new DatabaseSync(dbPath);
  ...
  const now = Date.now();
  const insS = db.prepare('INSERT INTO sessions VALUES (?,?,?,?,?,?,?,?,?,?)');
  const insM = db.prepare('INSERT INTO messages (id, session_id, role, content, timestamp, finish_reason) VALUES (?,?,?,?,?,?)');
  insS.run('hermes-loop-main', '主会话', 'C:/w', 'default', null, null, now - 5000, 'task_done', now - 5000, now - 60000);
  insM.run(1, 'hermes-loop-main', 'user', '分析仓库', now - 60000, null);
  insM.run(2, 'hermes-loop-main', 'assistant', '分析完成', now - 5000, 'stop');
  insS.run('hermes-loop-child', null, 'C:/w', 'default', 'hermes-loop-main', '{"_delegate_from":"hermes-loop-main"}', now - 4000, 'agent_close', now - 4000, now - 10000);
  insM.run(3, 'hermes-loop-child', 'user', '子目标', now - 9000, null);
  insM.run(4, 'hermes-loop-child', 'assistant', '子结果', now - 4000, 'stop');
  db.close();
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-board-hermes-loop-data-'));
  const script = `
    const store = require(${JSON.stringify(path.join(__dirname, '..', 'store.js'))});
    const hermes = require(${JSON.stringify(path.join(__dirname, 'hermes.js'))});
    const received = [];
    store.setCompletionStabilizeMs('hermes', 25);
    store.onAgentCompletion((ev) => received.push(ev));
    hermes.scanDb(${JSON.stringify(dbPath)}, store);
  ...
  try {
    ...
    assert.deepEqual(received.map((ev) => [ev.provider, ev.sessionId]), [['hermes', 'hermes-loop-main']]);
  } finally {
