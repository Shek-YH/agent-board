...
const ID = 'deepseek';
const ROOT = SOURCE_PATHS.deepseek;
const ZSTD_FRAME_MAGIC = Buffer.from([0x28, 0xb5, 0x2f, 0xfd]);

// DeepSeek Desktop 的 session.jsonl.zstd 没有会话级终态：turn/end 只是「回合结束」
// （data.reason.kind=completed/aborted），会话会跨回合长期存活，因此本适配器不派生
// kind='turn_end' 完成信号（见 parseLines）。会话「已完成」完全由 checkDesktopIdle 停顿推断
// （文件 mtime 静止超过 DEEPSEEK_IDLE_MS + 最后真实消息为 assistant + DEBOUNCE 确认），
// 经 store 通用完成出口（deepseek 稳定窗 4000ms）广播一次完成事件。
// 下面阈值即该停顿推断的判定参数，不代表数据源里存在会话终态。
...
function fileToSessionId(file) {
  ...
  // 目录名才是权威身份：sessionId 必须与 ref（`deepseek:<dirname>`）严格一致，
  // 否则同一份文件会派生出两个 ref，卡片与状态各自挂在其中一个上。
  return path.basename(path.dirname(file));
}

// 惰性建 ref：sessionId 来自压缩内容里的 session 头，只有解压成功才拿得到。
// 一旦拿不到，就必须回退到文件目录名——绝不能产出 `deepseek:` 这种空 sessionId 的畸形 ref
// （实测它会进 liveRefs + 卡片列表，渲染出一张永远「进行中」的假卡片）。
// file 为 null（如单测只调 parseLines）时返回 null，由调用方决定丢弃还是兜底。
function refForFile(file) {
  const id = file ? fileToSessionId(file) : '';
...
function parseLines(lines, file = null) {
  const out = [];
  const topology = topologyForSession(lines);
  let project = '';
  // 身份以文件目录名（session-<uuid>）为权威：压缩内容里的 session 头只是补充信息。
  // 反过来（信任内容头）会出两种故障：① 头被截断时 sessionId 为空 → 产出畸形 ref
  // `deepseek:server:<uid>:<uuid>`，同一份文件由此分裂成两个 ref，各挂一张卡片；
  // ② 头缺失时完全拿不到身份。目录名两者皆无。
  let sessionId = file ? fileToSessionId(file) : '';
  let title = '';
  let titleSource = '';
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (type === 'session') {
        project = line.cwd || '';
        // 内容头里的 id 仅在不与目录名冲突时采纳，避免上面那种分裂。
        const headerId = String(line.id || '').trim();
        if (!sessionId && headerId) sessionId = headerId;
        continue;
      }
      ...
      // DeepSeek Harness 的 turn/end 是「每回合结束」标记（data.reason.kind=completed/aborted），
      // 只表示当前回合结束，不代表会话结束——DSH 会话会跨回合长期存活、继续运行
      // （一次 user→assistant 交换写完 turn/end 后，下一轮会继续在同一个 session 上跑）。
      // 若把它当会话终态信号（kind:'turn_end' → setDoneSignal → doneSignalAt），会让一条
      // 仍在运行的会话在最近一轮结束后就被看板标成「已完成」，直到下一条更新的真实消息才解除。
      // 因此这里不产出 turn_end 完成事件；会话的「进行中/已完成」改由
      // checkDesktopIdle（文件 mtime 新鲜即 active、静止+末条为 assistant 才判完成）
      // 配合「最后真实消息 <10min」窗口驱动，与其它常驻桌面 agent 一致。
      if (type === 'turn/end') {
        continue;
      }
      ...
      if (type === 'user/message' && line.data) {
        ...
        if (!sessionId) continue; // 无法归属会话的消息一律丢弃，不产出空 ref
  ...
  if (title && sessionId) {    out.unshift({
      agent: ID,
...
function checkDesktopIdle(store) {
  ...
  for (const f of files) {
    ...
    // 只对 store 里确实存在的会话做停顿推断：目录名派生的 ref 若在 store 里没有对应会话，
    // 说明该会话的消息尚未被任何一次扫描成功解析（例如解压失败、内容头缺失）。
    // 此时无论怎么判「完成」都会写到一个孤立 ref 上，凭空造出畸形卡片——直接跳过。
    const ref = refForFile(f);
    if (!ref || !store.getSession(ref)) continue;
    ...
    if (!lastRole) {
      ...
      const msgs = parseLines(lines, f).filter((m) => m.kind === 'message');
...
module.exports = {
  ...
  DEEPSEEK_IDLE_MS, DEEPSEEK_FRESH_MS, DEEPSEEK_DEBOUNCE_MS, refForFile,
