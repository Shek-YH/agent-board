...
function topologyForSession(lines = []) {
  ...
  // The event type is an explicit durable identity signal. A parent is still
  // required so a malformed/partial artifact cannot create an orphan child.
  const hasDescriptor = lines.some((line) => {
    if (!line || line.type !== 'subagent/descriptor') return false;
    const data = line.data && typeof line.data === 'object' ? line.data : {};
    return [line.id, line.subagentId, line.subagent_id,
      data.id, data.subagentId, data.subagent_id, data.sessionId, data.session_id]
...
module.exports = {
  ID, ROOT, isSessionFile, fileToSessionId, parseLines, topologyForSession, tailRead, detect,
