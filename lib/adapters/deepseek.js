...
function scopedSourceId(sessionId, kind, value) {
  const raw = `${kind}:${value}`;
  return sessionId ? `${sessionId}:${raw}` : raw;
}
    ...
    try {
      ...
      if (type === 'turn/end' && line.data) {
        ...
        if (sessionId && reasonKind) {
          out.push({
            agent: ID,
            sourceId: scopedSourceId(sessionId, 'turnend', line.seq != null ? line.seq : (line.time || out.length)),
            sessionId,
      ...
      if (type === 'user/message' && line.data) {
        ...
        out.push({
          agent: ID,
          sourceId: scopedSourceId(sessionId, 'u', line.seq != null ? line.seq : Math.random()),
          sessionId,
          ts: Number(line.time) || 0,
          role: 'user',
          kind: 'message',
      ...
      if (type === 'assistant/message' && line.data && line.data.message) {
        ...
        out.push({
          agent: ID,
          sourceId: scopedSourceId(sessionId, 'a', line.seq != null ? line.seq : Math.random()),
          sessionId,
          ts: Number(line.time) || 0,
          role: 'assistant',
          kind: 'message',
...
module.exports = {
  ...
  scanAll(store) {
    ...
    for (const f of files) {
      ...
      // v4：首次升级时强制重扫一次旧文件，把历史 session-scoped source ID 补入 store。
      const mkey = `mtime:v4:${ID}:${f}`;
