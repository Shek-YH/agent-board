...
function parseLines(lines, filePath = '') {
  ...
  for (const line of lines) {
    try {
      ...
      // WorkBuddy writes the final assistant output with status=completed. Treat
      // that provider-level marker as an explicit turn end so child cards do not
      // wait for the desktop idle heuristic (40s + debounce) to become done.
      if (line.role !== 'user' && String(line.status || '').trim().toLowerCase() === 'completed') {
        out.push({
          agent: ID,
          sourceId: `${sourceId}:turnend`,
          sessionId: topology.sessionId,
          ts,
          kind: 'turn_end',
          project: line.cwd || '',
