...
function parseLines(lines, filePath = '') {
  ...
  for (const line of lines) {
    try {
      ...
      out.push({
        agent: ID,
        sourceId: line.id || (topology.sessionId + ':' + line.timestamp + ':' + Math.random()),
...
function fileToSessionId(file) {
  const fallback = path.basename(file).replace(/\.jsonl$/, '');
  if (!CHILD_FILE_RE.test(String(file || ''))) return fallback;
  return workbuddyFileTopology(file, { sessionId: readFirstSessionId(file) }).sessionId || fallback;
}
...
module.exports = {
  ID, ROOT, isSessionFile, parseLines, workbuddyFileTopology, scanHeartbeats, scanSessionStatuses, checkDesktopIdle, fileToSessionId, detect,
