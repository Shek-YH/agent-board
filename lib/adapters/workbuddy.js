...
// 且没有「正在生成」状态文件；jsonl 只写「已完成」的消息（status=completed，无流式中间态）→
// 「文件停止写入 + 最后一条真实消息是 assistant」是 agent 停笔的近似信号。
// 注意：agent 长思考间隙（实测一轮回复内全行间隙几乎都 < 130s，偶发 ~2.5 分钟）文件同样无写入，
// 所以阈值不能太激进——240s 覆盖所有实测工作场景，避免误判；剩余偶发长间隙会误判一次但
// 会在下次写入时自愈（单调事件，不是窗口边缘的反复横跳）。
const DESKTOP_IDLE_MS = 240 * 1000;   // 文件静止超过该时长 → 进入「疑似停笔」
const DESKTOP_FRESH_MS = 30 * 1000;   // 文件 30s 内有写入 → agent 正在跑
const DESKTOP_DEBOUNCE_MS = 40 * 1000; // 连续 2 次（20s 定时器）判定疑似停笔才真正判停，防瞬态抖动
...
function scanHeartbeats(store) {
  ...
  for (const name of entries) {
    ...
    try {
      ...
      // → 之后心跳不再前进 = 进程退出 → 该 session 提前结束「进行中」（不必等满 10 分钟消息窗口）
      const prevHb = lastHb.get(ref);
      const fresh = now - hb < 90 * 1000;
      if (prevHb === undefined) {
        lastHb.set(ref, hb); // 首次观察：只记录基准，不判定
      } else if (hb > prevHb && fresh) {
        hbTracked.add(ref);
        store.setAgentActive(ref, true, now);
        lastHb.set(ref, hb);
      } else {
        if (hbTracked.has(ref)) store.setAgentActive(ref, false, now); // 曾持续、现已停 → 提前完成
        lastHb.set(ref, hb);
      }
      // 心跳在 5 分钟内视为活跃
...
//   - mtime 距今 > 240s 且最后真实消息是 assistant 且文件末行是 message：
//       连续 2 次命中（40s debounce）→ setAgentActive(ref, false) → 立即退出「进行中」
//   - 其余（30~240s 中间地带 / 末行是 function_call|reasoning|ai-title 等 = agent 工作中）→ 不动
