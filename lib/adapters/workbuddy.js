...
// 转录没有独立的「本轮结束」事件。assistant message 的 status=completed 只表示这一条
// 消息生成完毕，后面可能紧跟工具调用，不能作为会话级完成信号。会话停笔仍使用
// 「文件停止写入 + 最后一条 assistant 消息」近似信号。
...
function parseLines(lines, filePath = '') {
  ...
  for (const line of lines) {
    try {
      ...
      if (line.type === 'function_call' && topology.sessionId) {
        out.push({
          agent: ID,
          sourceId: `activity:${line.id || (line.timestamp || out.length)}`,
