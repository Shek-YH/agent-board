...
const { createWorkBuddyMonitor, DEFAULT_CAPABILITIES } = require('../workbuddy-monitor');
...
const STATUS_SPOOL_PATHS = [...new Set([
  SOURCE_PATHS.workbuddySpool,
  path.join(os.homedir(), '.workbuddy-buddy', 'events.spool'),
])];
const statusReader = createStatusReader({ dbPath: SOURCE_PATHS.workbuddyDb });
let pluginHealthCache = { value: null, checkedAt: 0 };

function getPluginHealth() {
  if (pluginHealthCache.value && Date.now() - pluginHealthCache.checkedAt < 30 * 1000) return pluginHealthCache.value;
  pluginHealthCache = { value: inspectWorkBuddyPlugin(), checkedAt: Date.now() };
  return pluginHealthCache.value;
}

function createWorkBuddyStatusBridge({
  spoolPaths = STATUS_SPOOL_PATHS,
  maxReadBytes,
  stabilizationMs,
  pendingMaxMs,
  clock,
  ...
  const spoolReader = createSpoolReader({ spoolPaths, maxReadBytes });
  let monitor = null;
  let activeStore = null;
  let listeners = { onCompletion: () => {}, onFailure: () => {}, onDiagnostic: () => {} };

  function setListeners(next = {}) {
    listeners = {
      onCompletion: typeof next.onCompletion === 'function' ? next.onCompletion : () => {},
      onFailure: typeof next.onFailure === 'function' ? next.onFailure : () => {},
      onDiagnostic: typeof next.onDiagnostic === 'function' ? next.onDiagnostic : () => {},
    };
  }

  function ensureMonitor(store) {
    activeStore = store;
    if (monitor) return monitor;
    monitor = createWorkBuddyMonitor({
      clock,
      stabilizationMs,
      pendingMaxMs,
      capabilities,
      // L1 活体否决：confirmCompletion / revokePending 前询问 store「stop 之后是否仍有真实活动」。
      isLiveStill(sessionId, sinceTs) {
        return Boolean(activeStore?.isWorkBuddyStillActive?.(`${ID}:${sessionId}`, sinceTs));
      },
      onStatus(snapshot) {
        const target = activeStore;
        if (!target) return;
        const ref = `${ID}:${snapshot.sessionId}`;
        if (!target.getSession(ref)) {
          target.ingest({
            agent: ID,
            sourceId: `hook:${snapshot.sessionId}`,
            sessionId: snapshot.sessionId,
            ts: snapshot.lastEventAt || Date.now(),
            role: 'system',
            kind: 'heartbeat',
            text: '',
          });
        }
        target.noteWorkBuddyRuntimeStatus(ref, snapshot);
      },
      loadLastNotifiedCompletionId(sessionId) {
        return activeStore?.getWorkBuddyLastNotifiedCompletionId(`${ID}:${sessionId}`) || '';
      },
      saveLastNotifiedCompletionId(sessionId, completionId) {
        activeStore?.saveWorkBuddyLastNotifiedCompletionId(`${ID}:${sessionId}`, completionId);
      },
      onCompletion(event) {
        // monitor 在 onStatus 之后才会完成 store 更新；异步转发保证监听方读到的是新状态。
        queueMicrotask(() => listeners.onCompletion(event));
      },
      onFailure(event) {
        queueMicrotask(() => listeners.onFailure(event));
      },
      onDiagnostic(event) {
        listeners.onDiagnostic(event);
      },
    });
    // L2 反向撤销：store 侧 ingest 新真实消息 / SQLite 变 active / 心跳新鲜时会回调这里，
    // 撤销 monitor 中可能悬着的完成候选（防 stop 后仍在跑却误发完成）。ref 需去掉 agent 前缀。
    if (store && typeof store.setWorkBuddyCompletionRevoker === 'function') {
      store.setWorkBuddyCompletionRevoker((ref) => {
        const sid = String(ref || '');
        monitor.revokePending(sid.startsWith(`${ID}:`) ? sid.slice(ID.length + 1) : sid);
      });
    }
    return monitor;
  }

  function applyEvents(store, events, { baseline = false, readable = true } = {}) {
    const runtime = ensureMonitor(store);
    const inputs = Array.isArray(events) ? events : [];
    const states = runtime.handleBatch(inputs, { baseline });
    const staleCount = runtime.reconcileStale();
    return {
      ...
      completions: runtime.drainNotifications(),
      failures: runtime.drainFailures(),
    };
  }

  function scan(store, { baseline = false } = {}) {
    const result = spoolReader.read(store);
    return applyEvents(store, result.events, { baseline, readable: result.readable });
  }

  return {
    scan,
    ingest: (store, events, options) => applyEvents(store, events, options),
    reset: () => { spoolReader.reset(); monitor?.reset(); },
    setListeners,
    // 外部权威终态反向纠正 monitor 的卡死 waiting_user 状态（见 createWorkBuddyMonitor.noteExternalTerminal）
    noteExternalTerminal: (sessionId, ts) => monitor?.noteExternalTerminal(sessionId, ts) ?? false,
    getCapabilities: () => monitor?.getCapabilities() || { ...DEFAULT_CAPABILITIES, ...(capabilities || {}) },
  };
}

const statusBridge = createWorkBuddyStatusBridge({ capabilities: { httpHook: true } });
...
function isInternalHeartbeatSession(metadata = {}) {
  const sessionId = String(metadata.sessionId || '').trim().toLowerCase();
  const kind = String(metadata.kind || '').trim().toLowerCase();
  return kind === 'interactive' || kind === 'prewarm'
    || sessionId.startsWith('interactive-') || sessionId.startsWith('prewarm-');
}
...
function scanSessionStatuses(store, reader = statusReader) {
  ...
  for (const status of statuses.values()) {
    ...
    // monitor 可能卡在 waiting_user（待输入）而无后续解除事件；DB 已宣告终态时主动纠正，
    // 让 monitor 的 runtime_status 与权威终态一致（display 层 getWorkBuddyRuntimeStatus 也有兜底）。
    if (status.terminal) statusBridge.noteExternalTerminal(status.sessionId, status.statusAt);
  }
  return statuses.size;
}

// 心跳扫描：读取 ~/.workbuddy/sessions/*.json，报告每个活动会话
function scanHeartbeats(store, { heartbeatDir = HEARTBEAT_DIR, reader = statusReader } = {}) {
  const out = [];
  scanSessionStatuses(store, reader);
  if (!fs.existsSync(heartbeatDir)) return out;
  let entries;
  try { entries = fs.readdirSync(heartbeatDir); } catch { return out; }
  const now = Date.now();
  for (const name of entries) {
    const p = path.join(heartbeatDir, name);
    if (!name.endsWith('.json')) continue;
    try {
      const d = JSON.parse(fs.readFileSync(p, 'utf8'));
      if (!d.sessionId) continue;
      // WorkBuddy 启动/预热产生的内部 host 会话不是用户任务，不能进入活跃与完成提示链路。
      if (isInternalHeartbeatSession(d)) continue;
...
// 监听官方事件 spool 所在目录。目录尚未由插件创建时由 server 的 5s reconcile 兜底。
function watchStatusSpool(onEvent, spoolPaths = STATUS_SPOOL_PATHS) {
  const watchers = [];
  const directories = new Map();
  for (const spoolPath of spoolPaths || []) {
    if (typeof spoolPath !== 'string' || !spoolPath) continue;
    const absolute = path.resolve(spoolPath);
    directories.set(path.dirname(absolute), new Set([
      ...(directories.get(path.dirname(absolute)) || []),
      path.basename(absolute),
      `${path.basename(absolute)}.1`,
    ]));
  }
  for (const [directory, names] of directories) {
    ...
    try {
      const watcher = fs.watch(directory, { persistent: false }, (_event, name) => {
        ...
        try { onEvent(path.join(directory, name.toString())); } catch { /* fail-open */ }
      });
      watchers.push(watcher);
    } catch { /* 5s reconcile remains the fallback */ }
  }
  return () => {
...
function checkDesktopIdle(store) {
  ...
  for (const f of files) {
    ...
    if (store.getWorkBuddyRuntimeStatus?.(ref)) {
...
module.exports = {
  ...
  STATUS_SPOOL_PATHS, createWorkBuddyStatusBridge, watchStatusSpool,
  getPluginHealth,
  getStatusCapabilities() { return statusBridge.getCapabilities(); },
  ingestStatusEvents(store, events, options) { return statusBridge.ingest(store, events, options); },
  setStatusEventListeners(listeners) { statusBridge.setListeners(listeners); },
  scanStatusEvents(store, options) { return statusBridge.scan(store, options); },
  getHeartbeatTransition, isInternalHeartbeatSession,
  scanAll(store) {
