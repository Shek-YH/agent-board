...
test('GUI Agent 即使探测到 exe 也不伪装成 CLI 目标', () => {
  const result = buildLaunchTargets({
    defs: { workbuddy: { name: 'WorkBuddy' } },
    probes: { workbuddy: { installed: true, tier: 'gui', path: 'C:\\WorkBuddy\\WorkBuddy.exe' } },
    desktop: { workbuddy: { kind: 'scheme', available: true, value: 'workbuddy://' } },
  });
  assert.equal(result.workbuddy.cli.available, false);
  assert.equal(result.workbuddy.desktop.available, true);
});
