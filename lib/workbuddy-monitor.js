...
function createWorkBuddyMonitor(options = {}) {
  ...
  // 外部权威终态（workbuddy.db 已宣告 completed/terminated/...）反向纠正 monitor：
  // monitor 可能因 PermissionRequest / idle_prompt Notification 卡在 waiting_user（→ 待输入），
  // 而桌面会话结束常无后续解除事件（或 spool 已淘汰 SessionEnd）→ 状态永久滞留。
  // 这里以外部终态为准清除 waiting 标志并置 completed。单调保护：外部终态时刻早于 monitor
  // 已知最新事件时不覆盖（避免把「终态之后又有新活动」的会话误杀）。
  function noteExternalTerminal(sessionId, ts) {
    const runtime = sessions.get(String(sessionId || ''));
    if (!runtime) return false;
    if (runtime.status === 'completed' || runtime.status === 'failed') return false;
    const at = Number(ts) || Number(clock());
    if (runtime.lastEventAt && runtime.lastEventAt > at) return false;
    cancelCandidate(runtime);
