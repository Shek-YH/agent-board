...
test('外部 WorkBuddy active 状态在消息超过 10 分钟后仍出现在活跃快照', () => {
  store.clearAll();
  const sessionId = 'unit-long-running-workbuddy-active-snapshot';
  const ref = `workbuddy:${sessionId}`;
  const now = Date.now();
  store.ingest({
    agent: 'workbuddy', sourceId: 'unit-user-message-active-snapshot', sessionId,
    ts: now - 11 * 60 * 1000, role: 'user', kind: 'message', text: '生成长文件',
  });
  assert.equal(noteExternalStatus(ref, { active: true, statusAt: now }), true);

  assert.equal(store.getActive().some((item) => item.sessionRef === ref), true);
