...
function queryUninstallEntries(hive, runReg = defaultRunReg) {
  ...
  for (const line of String(r.stdout || '').split(/\r?\n/)) {
    ...
    const m = line.match(/^\s+(DisplayName|InstallLocation|DisplayIcon)\s+REG_(?:EXPAND_)?SZ\s+(.*)$/);
...
function registryExecutablePath(entry, executableNames, opts = {}) {
  ...
  if (displayIcon && (!names.length || normalizedNames.includes(path.basename(displayIcon).toLowerCase()))) {
...
function saveUserOverride(agent, executablePath, filePath = USER_OVERRIDES_PATH) {
  ...
  if (typeof executablePath === 'string' && executablePath.trim()) current[agent] = [executablePath.trim()];
...
function probeAgent(adapter, opts = {}) {
  ...
  const registryPath = registryExecutablePath(
    regHit,
    def.probe && def.probe.executableNames,
    opts,
  );

  if (pathHit || registryPath || regHit) {
    return {
      id, tier: def.tier, installed: true,
      path: pathHit || registryPath || null,
      executablePath: registryPath || (def.probe && def.probe.executable ? pathHit : null),
      registryName: regHit ? regHit.displayName : null,
      source: pathHit ? 'builtin' : (registryPath ? 'registry' : 'builtin'),
      version: tryVersion(verifyCmd),
    };
  }
...
module.exports = {
  ...
  loadUserOverrides, saveUserOverride, registryExecutablePath, tryVersion, probeAgent, probeAll,
