...
function canonicalDeepSeekSourceId(sourceId, sessionRef) {
  const raw = String(sourceId || '');
  if (!sessionRef.startsWith('deepseek:')) return raw;
  const sessionId = sessionRef.slice('deepseek:'.length);
  if (!sessionId || raw.startsWith(`${sessionId}:`)) return raw;
  return /^(u|a|turnend):/.test(raw) ? `${sessionId}:${raw}` : raw;
}
...
function load() {
  ...
  try {
    ...
    for (const m of data.messages || []) {
      ...
      m.source_id = canonicalDeepSeekSourceId(m.source_id, newRef);
...
const turnEndSeen = new Set(); // "agent:sessionId:sourceId"

// 统一入库入口
function ingest(msg) {
  const ref = msg.agent + ':' + msg.sessionId;
  const sourceId = canonicalDeepSeekSourceId(msg.sourceId, ref);
  ...
  if (msg.kind === 'turn_end') {
    // sourceId 可能只在单个会话内唯一（DeepSeek 的 seq 就是这种格式），
    // 去重键必须包含完整 session ref，不能让另一张卡的终态被吞掉。
  ...
  if (msg.kind === 'turn_start') {
    if (msg.agent === 'codex') noteCodexTurnEvent(ref, msg);
    let changed = false;
    // 某些 adapter 的完成信号可能只是工具调用前的一条中间回复；后续 turn_start
    // 证明会话仍在继续，应撤销此前的完成候选。
    const signalAt = doneSignalAt.get(ref);
    if (signalAt && (msg.ts || 0) > signalAt) { doneSignalAt.delete(ref); changed = true; }
    const stopAt = agentStopAt.get(ref);
    if (stopAt && (msg.ts || 0) > stopAt) { agentStopAt.delete(ref); changed = true; }
  ...
  if (!isMeta) {
    ...
    const nextMessage = {
      agent: msg.agent, source_id: sourceId, session_ref: ref,
...
function getWorkBuddyRuntimeStatus(ref) {
  return cloneRuntimeStatus(workbuddyRuntime.get(ref));
}
...
function getActive() {
  ...
  const now = nowMs();
  // lastMsgAt 过期清理后，仍由 WorkBuddy SQLite 标记为 active 的长任务不再有消息索引；
  // 把已有会话的外部 active ref 合并进来，避免 10 分钟后从 liveRefs 中消失。
...
// rescan / 管理用的高级 API
function clearAll() {
  sessions.clear(); messages.clear(); messagesBySession.clear(); meta.clear(); hidden.clear(); futCache.clear(); latestUserCache.clear(); userMsgFlag.clear(); lastMsgAt.clear(); lastRole.clear(); manualStatus.clear(); agentStopAt.clear(); doneSignalAt.clear(); externalDoneAt.clear(); externalActiveAt.clear(); workbuddyRuntime.clear(); pendingCodexDone.clear(); codexRuntime.clear(); turnEndSeen.clear();
  scheduleSave();
}
