...
function load() {
  ...
  try {
    ...
    sessions.clear(); messages.clear(); meta.clear(); hidden.clear(); futCache.clear(); userMsgFlag.clear(); lastMsgAt.clear(); lastRole.clear(); manualStatus.clear(); agentStopAt.clear(); doneSignalAt.clear(); pendingCodexDone.clear();
...
function setPendingCodexDone(ref, signalTs, holdMs) {
  pendingCodexDone.set(ref, { signalTs: signalTs || nowMs(), dueAt: nowMs() + holdMs });
}

// Codex 的任何新日志（包括无文本的 reasoning / custom_tool）都可取消上一轮 task_complete。
...
function isLiveRef(ref, now) {
  ...
  if ((!ts || ts < now - 10 * 60 * 1000) && !freshCodexLog) return false;
...
module.exports = {
  ...
  setAgentActive, setAgentStopped, isLiveRef, getLastRole, resolveAgentRef, setDoneSignal, noteCodexActivity, migrateCodexCompletionSignals,
