...
// Codex 运行证据的老化阈值：与 codex-status 共用同一条规则，避免「判定活跃」和「展示状态」两处口径漂移。
const CODEX_STALE_ACTIVE_MS = codexStatus.STALE_ACTIVE_MS ?? 10 * 60 * 1000;
...
const readComplete = new Map(); // ref -> true  该会话已读到过至少一条真实消息（判断卡片字段是否还是半成品）
...
// 「无身份 ref」：`<agent>:` 这种冒号后为空（或纯空白）的 ref。
// 它没有 sessionId，无法归属任何真实会话，却是一条 schema 合法的记录——能被写进快照、
// 能被加载回来，于是渲染成一张永远「进行中」的假卡片（实测发生过，msg_count 还涨到 369）。
// 成因是上游 adapter 在拿不到 sessionId 时仍旧派生了 ref，配合 ENOSPC / rename EPERM
// 期间的半截写入一起落盘。这里作为加载侧的统一闸门：畸形记录不得进入内存索引。
function isIdentitylessRef(ref) {
  const raw = String(ref || '');
  const sep = raw.indexOf(':');
  if (sep < 0) return false; // 无 agent 前缀的裸 ref 由其它逻辑处理，不在此拦
...
function load() {
  ...
  try {
    ...
    for (const s of data.sessions || []) {
      ...
      // 无身份的 ref（`deepseek:` 这种 agent 冒号后为空）不得进入内存索引。
      // 它是一条「早于 ingest 防御就写进快照」的历史脏数据：schema 合法所以能加载，
      // 但没有任何 sessionId 可供归属，会在看板上渲染出一张永远「进行中」的假卡片。
      // 在加载这一道唯一入口上拒绝它，历史脏数据与未来任何来源的脏数据同时被挡掉。
      if (isIdentitylessRef(oldRef)) continue;
    ...
    for (const m of data.messages || []) {
      ...
      // 与上面 sessions 循环同一道闸门：脏 ref 的残留消息不能重建出孤儿会话桶，
      // 否则卡片没了、消息还在，前端一渲染就又是一张无身份的「进行中」卡片。
      if (isIdentitylessRef(oldRef)) continue;
      const newRef = canonicalSessionRef(oldRef);
...
// 统一入库入口
function ingest(msg) {
  // 空 sessionId 会产出 "deepseek:" 这种畸形 ref：它会同时进 liveRefs 和卡片列表，
  // 渲染出一张永远「进行中」的假卡片（实测已发生）。没有 sessionId 的消息无法归属任何会话，
  // 直接丢弃——各 adapter 的解析层负责保证 sessionId 非空。
  if (!msg || !msg.agent || !String(msg.sessionId == null ? '' : msg.sessionId).trim()) return null;
  ...
  if (!s) {
    ...
    // 仅在「归一化后确实不同」时才改写 project。
    // Windows 路径大小写不敏感，同一目录会被不同 Agent 以不同大小写上报（C:\Foo 与 c:\Foo）。
    // 若按原始字符串比较，同一会话的 project 会在两种写法间反复横跳：既标脏触发无谓落盘，
    // 又让项目筛选把同一目录拆成多条。比较口径统一走 normalizeProjectPath。
    if (msg.project && normalizeProjectPath(s.project) !== normalizeProjectPath(msg.project)) {
      s.project = msg.project; changed = true;
    }
...
// lifecycleRuntime 是「软」证据：它只有事件驱动，**状态的保鲜由 engine 的 staleMs 老化负责**，
// 且来源 spool/replay 可能落后于真实状态。因此它只能**升级**结论、不能把已确认的终态改回进行中——
// 旧实现无条件覆盖 state，导致 390 小时前的 ACTIVE 把早已完成的卡片长期显示成「进行中」。
//
// 反向同理：当生命周期已经老化（publicState 变成 IDLE 且带 stale 标记）时，说明它最后的证据
// 已经过期，这个 IDLE **不能**用来支撑底层 status 的 running —— 否则一个早就不动的会话
// 会因为底层 lastEventAt 尚未被清理而继续显示「进行中」。
function withLifecycleStatus(ref, status) {
  const lifecycle = lifecycleRuntime.snapshot(ref);
  const lifecycleState = LIFECYCLE_TO_RUNTIME_STATE[lifecycle.publicState];
  const lifecycleFields = {
    lifecycle_state: lifecycle.publicState,
    lifecycle_updated_at: lifecycle.updatedAt,
  };
  if (!lifecycleState || lifecycle.publicState === 'UNKNOWN') return status;
  // 无底层 status 且生命周期只是「老化出来的 IDLE」时，不要凭空合成一条 idle 记录：
  // 该会话本就没有运行状态来源（例如只有 lastMsgAt、没有 workbuddyRuntime 条目），
  // 合成出来的 idle 会覆盖调用方按 liveRefs 推导的 running，导致真正在跑的卡片状态为空。
  if (!status && lifecycle.stale) return status;
  const lifecycleLive = lifecycle.publicState === 'ACTIVE' || lifecycle.publicState === 'COMPLETION_CANDIDATE';
  const statusLive = status ? status.state === 'running' : null;
  // 终态优先：外部数据库 / doneSignalAt / manualStatus 已判定的终态不允许被生命周期信号覆盖。
  if (status && !lifecycleLive && statusLive === false) {
    return { ...status, ...lifecycleFields };
  }
  // 证据已过期：底层仍声称 running，但生命周期证据已被 staleMs 判为失效 →
  // 不再展示「进行中」，降级为 idle（保留 status 其余字段供诊断）。
  if (status && lifecycle.stale && lifecycle.publicState === 'IDLE' && statusLive === true) {
    return { ...status, state: 'idle', ...lifecycleFields };
  }
  return {
    ...(status || {
...
function runtimeStatusFor(ref, now = nowMs()) {
  if (ref.startsWith('workbuddy:')) {
    ...
    return {
      ...
      currentTurnId: v2.currentTurnId,
    };
  }
...
function getRuntimeStatuses(now = nowMs()) {
  ...
  // 兜底：真正在跑的会话（权威 liveRefs）必须有状态，否则前端卡片会渲染成空状态。
  // 这类会话可能只有 lastMsgAt（没有 workbuddyRuntime / codexRuntime / stateEngine 条目），
  // 上面几个循环都覆盖不到它。
  for (const ref of liveRefSet(now)) {
    if (statuses[ref]) continue;
    const status = runtimeStatusFor(ref, now);
    statuses[ref] = withLifecycle(ref, status || { state: 'running', source: 'live_window' });
  }
  return statuses;
}

// 权威「进行中」集合（isLiveRef 口径）。getActive 会做一次 Map 清理，这里复用它保持一致。
function liveRefSet(now) {
  const refs = new Set();
  for (const ref of lastMsgAt.keys()) {
    if (isLiveRef(ref, now)) refs.add(ref);
  }
  for (const ref of externalActiveAt.keys()) {
    if (sessions.has(ref) && isLiveRef(ref, now)) refs.add(ref);
  }
  for (const ref of heartbeatActiveAt.keys()) {
    if (isLiveRef(ref, now)) refs.add(ref);
  }
  for (const ref of workbuddyRuntime.keys()) {
    if (isLiveRef(ref, now)) refs.add(ref);
...
//   5. Codex 的日志证据与 codex-status 同口径：threadStatus=active 但 lastEventAt 超过
//      STALE_ACTIVE_MS 时，codex-status 输出 stale_active（=「状态待确认」，非进行中）。
//      判定活跃必须复用同一条老化规则，否则「死在日志里的」codex 会话会进 liveRefs 却不进任何
//      真正的活跃状态，导致 liveRefs 与 status 层互相矛盾。
function isLiveRef(ref, now) {
  const workbuddy = workbuddyRuntime.get(ref);
  if (workbuddy) return workbuddy.state === 'running';
  const ts = lastMsgAt.get(ref);
  const freshCodexLog = ref.startsWith('codex:') && isFreshCodexEvidence(ref, now);
...
// Codex 运行证据是否仍然「新鲜」。与 lib/codex-status.js 的 getCodexDisplayStatus 保持一致：
// - activeMap.lastActivity 必须落在活跃窗口内（容忍思考/工具执行间隙）
// - codexRuntime 里 threadStatus=active 的记录，还要看它自己的 lastEventAt 是否已超过老化阈值；
//   否则一条早就没动静的 threadStatus=active 会永久把会话顶成「进行中」。
function isFreshCodexEvidence(ref, now) {
  const lastActivity = activeMap.get(ref)?.lastActivity || 0;
  const activityFresh = lastActivity >= now - 10 * 60 * 1000;
  const state = codexRuntime.get(ref);
  if (!state) return activityFresh;
  if (state.threadStatus !== 'active') return activityFresh;
  const lastEventAt = Number(state.lastEventAt) || 0;
  if (!lastEventAt) return activityFresh;
  const codexEvidenceFresh = now - lastEventAt <= CODEX_STALE_ACTIVE_MS;
  return activityFresh && codexEvidenceFresh;
}
...
function getTimeline({ agent = '', project = '', q = '', cursor = 0, limit = 100 } = {}) {
  ...
  // 项目过滤统一按归一化路径比较（大小写/末尾分隔符不敏感），否则同一目录会因写法不同而漏筛。
  ...
  for (const m of messages.values()) {
    ...
    if (projectKey && normalizeProjectPath(s.project) !== projectKey) continue;
...
function getSession(ref) {
  ...
  return {
    ...
    is_read_complete: isReadComplete(ref),
    messages: msgs,
  };
}
...
function getSessions({ agent = '', project = '', q = '', cursor = 0, limit = 100, onlyUser = false, since = 0 } = {}) {
  ...
  // 项目筛选按归一化路径比较：下拉框里的显示写法可能与个别会话实际存储的大小写不同，
  // 严格相等会把同一目录的会话漏掉一半（这正是「同一项目被拆开显示」的另一半原因）。
  if (project) {
    const projectKey = normalizeProjectPath(project);
    list = list.filter(s => normalizeProjectPath(s.project) === projectKey);
  }
  ...
  return list.slice(start, start + l).map(s => {
    ...
    return {
      ...
      // 数据是否已读到真实消息。false = 该卡仍可能缺 project / msg_count / 末条用户消息，
      // 前端据此显示「数据读取中」而不是把「还没读到」渲染成「（无项目路径）」和虚假条数。
      is_read_complete: isReadComplete(s.id),
    };
  });
}
...
function repairUserQueries() {
  ...
  // 重建读入完备标记：只认 user/assistant 这类真实消息，heartbeat/title 不算。
  readComplete.clear();
  for (const m of messages.values()) {
    if ((m.role === 'user' || m.role === 'assistant') && m.text) readComplete.set(m.session_ref, true);
  }
  scheduleSave();
}
...
// 会话是否已读到过「真实消息」。
//
// 关键：这个判定必须从内存里的 messages 索引实时推导，不能只靠 ingest 时打的运行时标记——
// store 启动时全量加载 data.json，此后 scanAll 会因 mtime 未变而跳过已扫过的文件，
// 于是「上次运行期间写入的标记」在重启后全部丢失，所有历史会话都会退化成「数据未就绪」。
// 判定用 role 而非「桶非空」：heartbeat / title 这类元消息也会进 messagesBySession，
// 只有 role 为 user/assistant 才是真正有内容的转录。
function isReadComplete(ref) {
  if (readComplete.get(ref) === true) return true;
  const bucket = messagesBySession.get(ref);
  if (!bucket || !bucket.size) return false;
  for (const m of bucket.values()) {
    if (m.role === 'user' || m.role === 'assistant') return true;
  }
  return false;
}
function markReadComplete(ref) {
  if (readComplete.get(ref)) return;
  readComplete.set(ref, true);
}
...
const stmts = {
  ...
  projects: { all: () => {
    // 按「归一化路径」分组，而不是原始字符串。
    // 各 Agent 上报 cwd 时大小写不统一（C:\Foo / c:\Foo），若用原始串做键，
    // 同一个项目文件夹会被拆成两条下拉项、会话也随之分成两组展示。
    // 显示名取出现次数最多的那种写法（次数相同取字典序更小者 —— 大写盘符天然排在前面），
    // 前端选中后回传该显示名，后端比较一律走 normalizeProjectPath，保证筛选不漏。
    const groups = new Map();
    for (const s of sessions.values()) {
      if (!s.project || hidden.has(s.id)) continue;
      const key = normalizeProjectPath(s.project);
      let g = groups.get(key);
      if (!g) { g = { variants: new Map(), cnt: 0, lastSeen: 0 }; groups.set(key, g); }
      g.cnt += 1;
      g.lastSeen = Math.max(g.lastSeen, s.last_seen || 0);
      g.variants.set(s.project, (g.variants.get(s.project) || 0) + 1);
    }
    return [...groups.values()].map((g) => {
      let display = '';
      let best = -1;
      for (const [name, n] of g.variants) {
        if (n > best || (n === best && name < display)) { display = name; best = n; }
      }
      return { project: display, cnt: g.cnt, lastSeen: g.lastSeen };
...
module.exports = {
  ...
  getStateEngineStatuses,  getStateEngineDiagnostics,
