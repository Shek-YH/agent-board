...
function load() {
  ...
  try {
    ...
    for (const task of data.todoTasks || data.todo_tasks || []) {
    ...
    for (const g of data.promptGroups || []) {
    ...
    for (const e of data.indexEntries || []) {
    ...
    if (Array.isArray(data.indexCategoryOrder)) indexCategoryOrder = data.indexCategoryOrder.map((c) => String(c || '').trim()).filter(Boolean);
...
function save() {
  ...
  saveInFlight = prevBackup
    .then(() => fs.promises.writeFile(tmp, json, 'utf-8'))
...
function getWorkBuddyRuntimeStatus(ref) {
  ...
  return manualStatus.has(ref) && status
    ? { ...status, state: 'completed', completedAt: status.completedAt || nowMs() }
    : status;
}
...
function runtimeStatusFor(ref, now = nowMs()) {
  if (ref.startsWith('workbuddy:')) return getWorkBuddyRuntimeStatus(ref);
  if (ref.startsWith('codex:')) return getCodexRuntimeStatus(ref, now);
  return null;
}
...
function getRuntimeStatuses(now = nowMs()) {
  ...
  for (const ref of workbuddyRuntime.keys()) {
    if (!statuses[ref]) statuses[ref] = getWorkBuddyRuntimeStatus(ref);
  }
  return statuses;
}
...
function createTodoTask(data) { const result = todoService.createTask(data); scheduleSave(); return result; }
function updateTodoTask(id, data) { const result = todoService.updateTask(id, data); scheduleSave(); return result; }
function setTodoTaskCompleted(id, completed) { const result = todoService.setTaskCompleted(id, completed); scheduleSave(); return result; }
function deleteTodoTask(id) { const result = todoService.deleteTask(id); scheduleSave(); return result; }
...
function createPromptGroup(data) { const r = promptService.createGroup(data); scheduleSave(); return r; }
function updatePromptGroup(id, data) { const r = promptService.updateGroup(id, data); scheduleSave(); return r; }
function deletePromptGroup(id) { const r = promptService.deleteGroup(id); scheduleSave(); return r; }
function getPromptGroups() { return promptService.getGroups(); }
function createPrompt(data) { const r = promptService.createPrompt(data); scheduleSave(); return r; }
function updatePrompt(id, data) { const r = promptService.updatePrompt(id, data); scheduleSave(); return r; }
function deletePrompt(id) { const r = promptService.deletePrompt(id); scheduleSave(); return r; }
function recordPromptUse(id) { const r = promptService.recordUse(id); scheduleSave(); return r; }
...
function createIndexEntry(data) { const r = indexService.createEntry(data); scheduleSave(); return r; }
function updateIndexEntry(id, data) { const r = indexService.updateEntry(id, data); scheduleSave(); return r; }
function deleteIndexEntry(id) { const r = indexService.deleteEntry(id); scheduleSave(); return r; }
function reorderIndexEntries(orderedIds) { const r = indexService.reorderEntries(orderedIds); scheduleSave(); return r; }
function setIndexCategoryOrder(order) { const r = indexService.setCategoryOrder(order); scheduleSave(); return r; }
