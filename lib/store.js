...
function getActive() {
  ...
  for (const ref of externalActiveAt.keys()) refs.add(ref);
...
// WorkBuddy 的交互心跳代表桌面会话进程仍存活；它优先覆盖同一时刻之前的
// SQLite completed 快照，但不覆盖显式 /api/complete 或手动完成信号。
function noteHeartbeat(ref, alive, observedAt = nowMs()) {
  ...
  if (ref.startsWith('workbuddy:')) {
    const externalAt = externalDoneAt.get(ref) || 0;
    if (externalAt && ts >= externalAt) externalDoneAt.delete(ref);
  }
  scheduleSave();
  return true;
}
