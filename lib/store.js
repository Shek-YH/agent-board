...
function getWorkBuddyRuntimeStatus(ref) {
  ...
  if (manualStatus.has(ref) && status) {
    return { ...status, state: 'completed', completedAt: status.completedAt || nowMs() };
  }
  ...
  if (status && externalDoneAt.has(ref) && !EXTERNAL_TERMINAL_STATES.has(status.state)) {
    return { ...status, state: 'completed', completedAt: externalDoneAt.get(ref) };
  }
  return status;
}
...
function getRuntimeStatuses(now = nowMs()) {
  ...
  for (const [ref, session] of sessions) {
    if (session.agent === 'codex') statuses[ref] = withLifecycle(ref, getCodexRuntimeStatus(ref, now));
  }
  for (const ref of codexRuntime.keys()) {
    if (!statuses[ref]) statuses[ref] = withLifecycle(ref, getCodexRuntimeStatus(ref, now));
  }
  for (const ref of workbuddyRuntime.keys()) {
    if (!statuses[ref]) statuses[ref] = withLifecycle(ref, getWorkBuddyRuntimeStatus(ref));
  }
  return statuses;
}
