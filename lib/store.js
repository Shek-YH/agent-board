...
function load() {
  ...
  try {
    ...
    sessions.clear(); messages.clear(); meta.clear(); hidden.clear(); futCache.clear(); latestUserCache.clear(); userMsgFlag.clear(); lastMsgAt.clear(); lastRole.clear(); manualStatus.clear(); agentStopAt.clear(); doneSignalAt.clear(); externalDoneAt.clear(); pendingCodexDone.clear(); codexRuntime.clear();
...
// WorkBuddy 自带数据库的终态信号：只接受单调前进的状态时间，且旧状态不能覆盖更晚的真实消息。
function noteExternalStatus(ref, status) {
  ...
  if (status.active) {
    const previous = externalDoneAt.get(ref) || 0;
    if (previous && statusAt >= previous) return externalDoneAt.delete(ref);
  }
  return false;
}
