...
test('prepares the exact session before the first identity verification when requested', async () => {
  const calls = [];
  const result = await dispatchVerifiedMessage(
    { agent: 'codex', sessionRef: 'codex:session-1', message: 'hello' },
    baseDeps({
      prepareSession: async () => { calls.push('prepare'); return { ok: true, action: 'activated' }; },
      verifySession: async () => { calls.push('verify'); return { ok: true, strongAnchor: true, anchor: 'session-1' }; },
      activateSession: async () => { calls.push('activate'); return { ok: true }; },
    }),
  );
  ...
  assert.equal(result.ok, true);
  assert.deepEqual(calls.slice(0, 2), ['prepare', 'verify']);
  assert.deepEqual(phaseNames(result).slice(0, 6), [
    'PREPARE', 'LOCK', 'RESOLVE_SESSION', 'PREPARE_SESSION', 'VERIFY_SESSION', 'ACTIVATE',
  ]);
});
...
test('dispatch supports Claude and WorkBuddy when every capability is injected and verified', async () => {
  for (const agent of ['claude', 'workbuddy']) {
    const sessionRef = `${agent}:session-1`;
    const result = await dispatchVerifiedMessage(
      { agent, sessionRef, message: '检查测试' },
      baseDeps({
        resolveSession: async () => ({ status: 'resolved', target: resolvedTarget({ agent, sessionRef }) }),
      }),
    );
