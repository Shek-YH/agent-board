...
test('reports safe capability metadata without exposing implementations', () => {
  ...
  assert.deepEqual(report, {
    ...
    supportedAppVersionRange: 'unknown',
    selectorProfileVersion: 'unknown',
    lastProbeAt: null,
    lastProbeResult: 'not_probed',
    modelCatalogProbeAt: null,
...
test('keeps adapter version metadata separate from capability implementations and updates probe state safely', () => {
  const set = createCapabilitySet('codex', definitions(), {
    adapterVersion: '1.2.3',
    supportedAppVersionRange: '>=0.151.0',
    selectorProfileVersion: 'routing-2',
  });
  assert.deepEqual(set.report(), {
    ...
    lastProbeResult: 'not_probed',
    modelCatalogProbeAt: null,
    capabilities: set.report().capabilities,
  });

  const updated = set.updateMetadata({
    lastProbeAt: 1_700_000_000_000,
    lastProbeResult: 'installed',
    modelCatalogProbeAt: 1_700_000_000_100,
    implementation: 'must be ignored',
  });
  assert.equal(updated.lastProbeAt, 1_700_000_000_000);
  assert.equal(updated.lastProbeResult, 'installed');
  assert.equal(updated.modelCatalogProbeAt, 1_700_000_000_100);
  assert.equal('implementation' in updated, false);
});
