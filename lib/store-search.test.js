...
const { matchesSessionQuery, selectSessionCardUserText } = require('../test-support/store-fixture');
...
test('所有 agent 的 session 卡片都显示最近一条用户指令', () => {
  for (const agent of ['hermes', 'claude', 'codex', 'workbuddy', 'deepseek', 'marvis', 'zcode', 'pi']) {
    assert.equal(selectSessionCardUserText(agent, '首条指令', '最近指令'), '最近指令');
  }
  assert.equal(selectSessionCardUserText('codex', '首条指令', ''), '首条指令');
});
