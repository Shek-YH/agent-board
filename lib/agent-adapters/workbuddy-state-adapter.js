...
function signalForEvent(event) {
  const name = String(event.event || event.hook_event_name || '').trim();
  if (!name && event.kind === 'message') return event.role === 'user' ? EVENT_TYPES.USER_MESSAGE : EVENT_TYPES.ASSISTANT_MESSAGE;
  if (!name && event.kind === 'heartbeat') return EVENT_TYPES.TRANSCRIPT_ACTIVITY;
...
function hookEventToEvidence(event, options = {}) {
  ...
  const eventId = String(event.event_id || event.eventId || event.sourceId || `${sessionRef}:${event.event || event.kind || signalType}:${occurredAt}`).trim();
  const value = baseValue(event);
  if (event.session_role === 'child' || event.sessionRole === 'child') value.sessionRole = 'child';
  const source = event.kind === 'message' || event.kind === 'heartbeat'
    ? EVIDENCE_SOURCES.JSONL : EVIDENCE_SOURCES.NATIVE_HOOK;
  return normalizeEvidence({
    ...
    authority: Number(options.authority ?? (source === EVIDENCE_SOURCES.JSONL ? 90 : 100)),
  });
}
