...
test('Codex delivery removes line continuation escapes emitted by desktop input', () => {
  const { captureCodexDeliverySnapshot, verifyCodexDelivery } = require('./codex-delivery');
  const fixture = fixtureFile();
  try {
    const snapshot = captureCodexDeliverySnapshot(TARGET, { filePath: fixture.filePath, now: SENT_AT - 1000 });
    const transportText = 'hello' + '\\' + '\n' + 'world' + '\\' + '\n' + 'tail';
    fs.appendFileSync(fixture.filePath, JSON.stringify({
      timestamp: '2026-08-30T10:01:01.000Z',
      type: 'response_item',
      payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: transportText }] },
    }) + '\n');
    const result = verifyCodexDelivery(TARGET, 'hello\nworld\ntail', { snapshot, sentAt: SENT_AT });
    assert.equal(result.ok, true);
    assert.equal(result.delivered, true);
    assert.equal(result.transportNormalized, true);
  } finally {
...
test('Codex delivery reader waits for the appended message to become visible after SEND', async () => {
  const { createCodexDeliveryReader } = require('./codex-delivery');
  const fixture = fixtureFile();
  try {
    const reader = createCodexDeliveryReader({ filePath: fixture.filePath, deliveryWaitMs: 300, deliveryPollMs: 10 });
    const snapshot = reader.snapshot(TARGET, { now: SENT_AT - 1000 });
    const verification = reader.verify(TARGET, 'hello\nworld', { snapshot, sentAt: SENT_AT });
    setTimeout(() => fs.appendFileSync(fixture.filePath, JSON.stringify({
      timestamp: '2026-08-30T10:01:01.000Z',
      type: 'response_item',
      payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'hello\nworld' }] },
    }) + '\n'), 25);
...
test('Codex delivery can reconcile after a crash using only the instruction fingerprint', () => {
  const { captureCodexDeliverySnapshot, verifyCodexDeliveryFingerprint } = require('./codex-delivery');
  const fixture = fixtureFile();
  try {
    const snapshot = captureCodexDeliverySnapshot(TARGET, { filePath: fixture.filePath, now: SENT_AT - 1000 });
    fs.appendFileSync(fixture.filePath, JSON.stringify({
    ...
    const result = verifyCodexDeliveryFingerprint(TARGET, fingerprint, { snapshot, sentAt: SENT_AT });
    assert.equal(result.ok, true);
    assert.equal(result.delivered, true);
