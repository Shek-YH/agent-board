...
test('Pi 解析后即使源文件删除，fileToSessionId 仍返回缓存的 header ID', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-board-pi-session-cache-'));
  const file = path.join(dir, 'unique-cache-target.jsonl');
  const sessionId = '019f96fe-643b-76de-a797-24a9771496c1';
  fs.writeFileSync(file, `${JSON.stringify({ type: 'session', id: sessionId, cwd: dir })}\n`);

  try {
    pi.parseLines([{ type: 'session', id: sessionId, cwd: dir }], file);
    fs.rmSync(file);
    assert.equal(pi.fileToSessionId(file), sessionId);
  } finally {
...
test('Pi scanAll 和 poll 使用 topology-v2 offset，保证历史记录重投影', () => {
  assert.equal((PI_ADAPTER_SOURCE.match(/const key = fileOffsetKey\(f\);/g) || []).length, 2);
});
