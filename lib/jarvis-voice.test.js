...
function setup(fetchImpl, envOverrides = {}) {
  ...
  const env = { AGENT_BOARD_HEADLESS_EXECUTION: '1', ZAI_API_KEY: 'test-key', ...envOverrides };
...
test('voice runtime accepts ZHIPU_API_KEY as the shared provider alias', () => {
  const { runtime } = setup(undefined, { ZAI_API_KEY: '', ZHIPU_API_KEY: 'alias-key' });
  assert.equal(runtime.readiness().asr.available, true);
  assert.equal(runtime.readiness().supervisor.available, true);
  assert.equal(runtime.readiness().tts.available, true);
});
