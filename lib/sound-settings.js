...
function loadSoundSettings(paths) {
  try {
    ...
    if (!settings || typeof settings !== 'object' || Array.isArray(settings)
      ...
      || !Object.values(settings.assignments).every((id) => typeof id === 'string')) return emptySettings();
    return { assignments: { ...settings.assignments }, sounds: settings.sounds.map((sound) => ({ ...sound })) };
  } catch {
...
module.exports = { AUDIO_TYPES, MAX_AUDIO_BYTES, SOUND_DIR, SETTINGS_PATH, UPLOAD_DIR, assignSound, deleteSound, loadSoundSettings, uploadSound };
