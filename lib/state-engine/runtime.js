...
const { createSessionIdentity } = require('./identity');
...
function normalizeIdentity(sessionRef, input = {}) {
  ...
  return createSessionIdentity({ ...input, agent, hostId, generation: input.generation ?? 0 });
}
...
function normalizeRuntime(input) {
  ...
  const runtimeSessionKey = String(
    input.runtimeSessionKey || (identity.nativeSessionId
      ? `${identity.agent}:${identity.nativeSessionId}:gen:${identity.generation}`
      : `${sessionRef}:gen:${identity.generation}`),
  ).trim();
  ...
  const result = {
    ...
    completionNotificationIds: uniqueIds(input.completionNotificationIds),
    recentEvidence,
...
function cloneRuntime(runtime, patch = {}) {
  ...
  if (patch.identity && patch.runtimeSessionKey === undefined) {
    input.runtimeSessionKey = input.identity.nativeSessionId
      ? `${input.identity.agent}:${input.identity.nativeSessionId}:gen:${input.identity.generation}`
      : `${input.sessionRef}:gen:${input.identity.generation}`;
  }
