...
test('createInitialRuntime creates all canonical dimensions and a generation-scoped key', () => {
  const runtime = createInitialRuntime({
    ...
    identity: { agent: 'codex', hostId: 'host-1', nativeSessionId: 'session-1', generation: 4 },
    updatedAt: 120,
  });

  assert.equal(runtime.sessionRef, 'codex:session-1');
  assert.equal(runtime.runtimeSessionKey, 'codex:session-1:gen:4');
  assert.deepEqual(runtime.identity, {
    agent: 'codex', hostId: 'host-1', nativeSessionId: 'session-1', generation: 4,
  });
