...
test('外部权威终态纠正卡死的 waiting_user（待输入）', () => {
  const { monitor } = makeMonitor();
  monitor.handle(event('UserPromptSubmit', 100));
  monitor.handle(event('PermissionRequest', 110));
  assert.equal(monitor.get('session-1').status, 'waiting_user');
  // workbuddy.db 宣告 completed，且终态时刻晚于已知事件
  const changed = monitor.noteExternalTerminal('session-1', 200);
  assert.equal(changed, true);
  assert.equal(monitor.get('session-1').status, 'completed');
  assert.equal(monitor.get('session-1').waitingForPermission, false);
});

test('外部终态早于 monitor 已知事件时不覆盖（保守）', () => {
  const { monitor } = makeMonitor();
  monitor.handle(event('UserPromptSubmit', 100));
  monitor.handle(event('PermissionRequest', 300));
  // 终态时刻 200 早于最新事件 300 → 不覆盖，仍是 waiting_user
  assert.equal(monitor.noteExternalTerminal('session-1', 200), false);
  assert.equal(monitor.get('session-1').status, 'waiting_user');
});

test('noteExternalTerminal 对未知会话返回 false（不创建幽灵 runtime）', () => {
  const { monitor } = makeMonitor();
  assert.equal(monitor.noteExternalTerminal('never-seen', 200), false);
  assert.equal(monitor.get('never-seen'), undefined);
});
