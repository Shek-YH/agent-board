...
const CAPABILITY_METADATA_NAMES = Object.freeze([
  'adapterVersion',
  'supportedAppVersionRange',
...
function normalizeCapabilityMetadata(agentId, metadata = {}) {
  const input = metadata && typeof metadata === 'object' && !Array.isArray(metadata) ? metadata : {};
  return {
    agentId,
    adapterVersion: text(input.adapterVersion) || 'unknown',
    supportedAppVersionRange: text(input.supportedAppVersionRange) || 'unknown',
    selectorProfileVersion: text(input.selectorProfileVersion) || 'unknown',
    lastProbeAt: normalizeTimestamp(input.lastProbeAt),
    lastProbeResult: text(input.lastProbeResult) || 'not_probed',
    modelCatalogProbeAt: normalizeTimestamp(input.modelCatalogProbeAt),
  };
}

function createCapabilitySet(agentId, definitions, metadata = {}) {
  ...
  let capabilityMetadata = Object.freeze(normalizeCapabilityMetadata(id, metadata));
  const set = {
    agentId: id,
    capabilities,
    has(name) {
    ...
    updateMetadata(patch = {}) {
      const next = { ...capabilityMetadata };
      if (patch && typeof patch === 'object' && !Array.isArray(patch)) {
        for (const name of CAPABILITY_METADATA_NAMES) {
          if (Object.prototype.hasOwnProperty.call(patch, name)) next[name] = patch[name];
        }
      }
      next.agentId = id;
      capabilityMetadata = Object.freeze(normalizeCapabilityMetadata(id, next));
      return set.report();
    },
    report() {
      return {
...
function createCapabilityRegistry(definitions) {
  ...
  for (const definition of definitions) {
    ...
    const set = createCapabilitySet(definition.agentId, definition.capabilities, definition.metadata);
  ...
  return Object.freeze({
    ...
    updateMetadata(agentId, patch = {}) {
      const set = sets.get(text(agentId));
      return set ? set.updateMetadata(patch) : null;
    },
  });
}
