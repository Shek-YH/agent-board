...
test('单任务单回合的 Codex task_complete 只进入 5 秒短观察期', () => {
  const events = codex.parseLines([
    { timestamp: '2026-08-23T03:03:31.000Z', type: 'event_msg', payload: { type: 'task_complete', turn_id: 'turn-demo' } },
...
test('Codex 回合事件保留 turnId，并识别中断回合', () => {
  const events = codex.parseLines([
    { timestamp: '2026-08-24T03:03:31.000Z', type: 'event_msg', payload: { type: 'task_started', turn_id: 'turn-started' } },
    { timestamp: '2026-08-24T03:03:32.000Z', type: 'event_msg', payload: { type: 'turn_aborted', turn_id: 'turn-aborted', reason: 'user_interrupt' } },
    { timestamp: '2026-08-24T03:03:33.000Z', type: 'event_msg', payload: { type: 'task_complete', turn_id: 'turn-complete' } },
  ], path.join('C:', 'sessions', 'rollout-2026-08-24T03-03-30-demo.jsonl'));

  assert.deepEqual(events.map(({ kind, turnId, turnStatus }) => ({ kind, turnId, turnStatus })), [
    { kind: 'turn_start', turnId: 'turn-started', turnStatus: 'inProgress' },
    { kind: 'turn_end', turnId: 'turn-aborted', turnStatus: 'interrupted' },
    { kind: 'turn_end', turnId: 'turn-complete', turnStatus: 'completed' },
  ]);
});

test('task_complete 携带失败状态时不误标为正常完成', () => {
  const events = codex.parseLines([
    { timestamp: '2026-08-24T03:04:31.000Z', type: 'event_msg', payload: { type: 'task_complete', turn_id: 'turn-failed', status: 'failed' } },
  ], path.join('C:', 'sessions', 'rollout-2026-08-24T03-04-30-failed.jsonl'));

  assert.equal(events[0].turnId, 'turn-failed');
  assert.equal(events[0].turnStatus, 'failed');
});
