...
test('用户级 source-paths.json 可覆盖非标准数据源位置', () => {
  ...
  const overrides = {
    ...
    workbuddy: {
      ...
      spool: 'G:\\AgentData\\workbuddy-events.spool',
    },
  ...
  assert.equal(paths.workbuddySpool, path.win32.resolve(overrides.workbuddy.spool));
  assert.equal(paths.hermesDb, path.win32.resolve(overrides.hermes.db));
});

test('WorkBuddy 官方事件 spool 支持环境变量并默认落到 Agent Board 数据目录', () => {
  const configured = resolveSourcePaths({
    platform: 'win32',
    homedir: 'C:\\Users\\alice',
    env: { AGENT_BOARD_WORKBUDDY_SPOOL_PATH: 'D:\\AgentData\\events.spool' },
  });
  assert.equal(configured.workbuddySpool, path.win32.resolve('D:\\AgentData\\events.spool'));

  const defaults = resolveSourcePaths({
    platform: 'win32',
    homedir: 'C:\\Users\\alice',
    env: { LOCALAPPDATA: 'F:\\Profiles\\alice\\Local' },
  });
  assert.equal(defaults.workbuddySpool, path.win32.resolve('F:\\Profiles\\alice\\Local\\AgentBoard\\workbuddy\\events.spool'));
});
