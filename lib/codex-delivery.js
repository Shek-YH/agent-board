...
async function waitForCodexDelivery(target, expectedMessage, context = {}, options = {}) {
  const timeoutMs = Number.isInteger(options.deliveryWaitMs) && options.deliveryWaitMs >= 0 ? options.deliveryWaitMs : 3_000;
  const pollMs = Number.isInteger(options.deliveryPollMs) && options.deliveryPollMs > 0 ? options.deliveryPollMs : 50;
  const deadline = Date.now() + timeoutMs;
  let result = verifyCodexDelivery(target, expectedMessage, context, options);
  while (result.code === 'DELIVERY_NOT_FOUND' && Date.now() < deadline) {
    await wait(Math.min(pollMs, Math.max(1, deadline - Date.now())));
    result = verifyCodexDelivery(target, expectedMessage, context, options);
  }
  return result;
}

async function waitForCodexDeliveryFingerprint(target, expectedFingerprint, context = {}, options = {}) {
  const timeoutMs = Number.isInteger(options.deliveryWaitMs) && options.deliveryWaitMs >= 0 ? options.deliveryWaitMs : 3_000;
  const pollMs = Number.isInteger(options.deliveryPollMs) && options.deliveryPollMs > 0 ? options.deliveryPollMs : 50;
  const deadline = Date.now() + timeoutMs;
  let result = verifyCodexDeliveryFingerprint(target, expectedFingerprint, context, options);
  while (result.code === 'DELIVERY_NOT_FOUND' && Date.now() < deadline) {
    await wait(Math.min(pollMs, Math.max(1, deadline - Date.now())));
    result = verifyCodexDeliveryFingerprint(target, expectedFingerprint, context, options);
  }
  return result;
}
...
function createCodexDeliveryReader(options = {}) {
  return {
    ...
    verify(target, expectedMessage, context = {}) {
      return waitForCodexDelivery(target, expectedMessage, context, options);
    },
    verifyFingerprint(target, expectedFingerprint, context = {}) {
      return waitForCodexDeliveryFingerprint(target, expectedFingerprint, context, options);
    },
  };
}
...
module.exports = {
  ...
  waitForCodexDelivery,
  waitForCodexDeliveryFingerprint,
