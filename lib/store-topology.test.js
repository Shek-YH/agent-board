...
test('历史重扫不会用当前扫描时刻把旧 Codex 会话顶为进行中', () => {
  const now = Date.now();
  const ref = 'codex:historical-rescan';
  store.ingest({
    agent: 'codex', sourceId: 'historical-rescan-message', sessionId: 'historical-rescan',
    ts: now - 11 * 60 * 1000, role: 'assistant', kind: 'message', text: '历史消息',
  });

  store.noteCodexActivity(ref, now - 11 * 60 * 1000, { agent: 'codex', project: '' });

  assert.equal(store.isLiveRef(ref, now), false);
});

test('历史重扫不会用旧 Codex turn_start 的当前处理时刻制造进行中', () => {
  const now = Date.now();
  const ref = 'codex:historical-turn-start';
  store.ingest({
    agent: 'codex', sourceId: 'historical-turn-start', sessionId: 'historical-turn-start',
    ts: now - 11 * 60 * 1000, kind: 'turn_start', turnId: 'old-turn',
  });
...
test('strict control target carries the same fallback title as the session card', () => {
  const now = Date.now();
  const project = 'C:/Projects/control-title-fallback';
  const sessionId = 'control-title-fallback';
  store.ingest({
    agent: 'codex', sourceId: 'control-title-fallback-message', sessionId,
    ts: now, role: 'user', kind: 'message', text: '请继续完成这个博客页面并先给出设计方案', project,
  });

  const card = store.getSessions({ agent: 'codex', project, limit: 10 })[0];
  const target = store.resolveSessionControlTarget({
    agent: 'codex', project, sessionRef: `codex:${sessionId}`,
  });

  assert.equal(target.status, 'resolved');
  assert.equal(target.candidates[0].title, card.title);
  assert.ok(target.candidates[0].title);
});
