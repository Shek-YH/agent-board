...
function applyCodexTurnEvent(state, event) {
  ...
  // store.ingest 里 kind='turn_end' 事件在 adapter 侧以 completionHoldMs 命名（codex.js parseLines
  // 的 task_complete 分支），store 原样转发给 applyCodexTurnEvent；这里两种字段名都接受，
  // 避免观察窗期间 runtime_status 提前显示 completed（卡片 live 状态仍由 pendingCodexDone 维持）。
  const holdMs = Number(event.completionHoldMs ?? event.holdMs) || 0;
  if (status === 'completed' && holdMs > 0) {
    next.threadStatus = 'active';
    next.monitorState = 'running';
    next.pendingCompletionAt = ts + holdMs;
