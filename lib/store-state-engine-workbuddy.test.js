...
test('Store forwards WorkBuddy hook, heartbeat and SessionEnd evidence to V2', () => {
  ...
  try {
    const script = [
      ...
      "const base = { provider: 'workbuddy', sessionId: 'workbuddy-session', activeToolIds: [], activeSubagentIds: [], lastStopHookActive: false };",
      ...
      "process.stdout.write(JSON.stringify({ mode: s.state_engine_mode, canonical: s.canonical_state, ui: s.ui_status.key }));",
    ].join('');
