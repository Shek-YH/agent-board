...
test('主会话和子代理提示音可以分别开关', () => {
  const paths = createPaths();
  try {
    const sound = uploadSound({ dataUrl: WAV_DATA_URL, name: '完成.wav' }, paths);
    assignSound('codex', sound.id, paths);

    const disabledChild = setSoundEnabled('codex', false, paths, 'child');
    assert.deepEqual(disabledChild.disabledAgentRoles, ['codex:child']);
    assert.equal(disabledChild.disabledAgents, undefined);
    assert.equal(disabledChild.assignments.codex, sound.id);

    const disabledMain = setSoundEnabled('codex', false, paths, 'main');
    assert.deepEqual(disabledMain.disabledAgentRoles, ['codex:child', 'codex:main']);

    const enabledChild = setSoundsEnabled(['codex'], true, paths, 'child');
    assert.deepEqual(enabledChild.disabledAgentRoles, ['codex:main']);

    const enabledAll = setSoundsEnabled(['codex'], true, paths);
    assert.equal(enabledAll.disabledAgentRoles, undefined);
    assert.equal(enabledAll.disabledAgents, undefined);
  } finally {
    fs.rmSync(paths.root, { force: true, recursive: true });
  }
