...
const {
  ...
  setSoundEnabled,
  setSoundsEnabled,
  setSubagentSoundEnabled,
  setSubagentSoundsEnabled,
  uploadSound,
} = require('./sound-settings');
...
test('子代理通知开关与普通完成声音绑定独立持久化', () => {
  const paths = createPaths();
  try {
    const sound = uploadSound({ dataUrl: WAV_DATA_URL, name: '完成.wav' }, paths);
    assignSound('codex', sound.id, paths);

    const disabled = setSubagentSoundEnabled('codex', false, paths);
    assert.deepEqual(disabled.disabledSubagentAgents, ['codex']);
    assert.equal(disabled.disabledAgents, undefined);
    assert.equal(disabled.assignments.codex, sound.id);

    const disabledAll = setSubagentSoundsEnabled(['codex', 'claude'], false, paths);
    assert.deepEqual(disabledAll.disabledSubagentAgents, ['claude', 'codex']);

    const enabledAll = setSubagentSoundsEnabled(['codex', 'claude'], true, paths);
    assert.equal(enabledAll.disabledSubagentAgents, undefined);
    assert.equal(loadSoundSettings(paths).assignments.codex, sound.id);
  } finally {
    fs.rmSync(paths.root, { force: true, recursive: true });
  }
});
