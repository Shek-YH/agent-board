...
const SLOTS = {
  ...
  supervisor_llm: ['DASHSCOPE_API_KEY', 'ZAI_API_KEY', 'ARK_API_KEY', 'MINIMAX_API_KEY', 'DEEPSEEK_API_KEY', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GEMINI_API_KEY', 'OPENROUTER_API_KEY'],
...
const SUPERVISOR_ENV_KEYS = Object.freeze({
  ...
  model: ['AGENT_BOARD_SUPERVISOR_MODEL', 'AUTOPILOT_SUPERVISOR_MODEL', 'JARVIS_SUPERVISOR_MODEL', 'SUPERVISOR_MODEL'],
...
function providerKeyNames(provider) {
  ...
  return normalized ? [`${normalized}_API_KEY`, 'AGENT_BOARD_SUPERVISOR_API_KEY'] : ['AGENT_BOARD_SUPERVISOR_API_KEY'];
}
...
function getProviderCapabilities(env = process.env) {
  ...
  for (const [slot, keys] of Object.entries(SLOTS)) {
    const key = keys.find((name) => String(source[name] || '').trim());
