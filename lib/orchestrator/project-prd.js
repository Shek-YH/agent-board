...
async function requestTaskCompiler(model, { goal = '', prdContent = '' } = {}, fetchImpl) {
  ...
  const timeoutTimer = setTimeout(() => controller.abort(), 60_000);
  try { response = await fetchImpl(model.endpoint, { method: 'POST', headers, body, signal: controller.signal }); } catch { throw new Error('AI Task Contract 编译请求失败'); } finally { clearTimeout(timeoutTimer); }
