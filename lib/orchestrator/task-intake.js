...
const GENERATION_SOURCES = Object.freeze(['user_goal', 'prd', 'model', 'safety_policy', 'inferred', 'user_edited']);
...
function normalizeTaskContract(value = {}) {
  ...
  const humanRequired = kind === 'high_risk' || humanInput.required === true || Boolean(needsHumanReason);
  ...
  return {
    ...
    humanGate: {
      ...
      state: ['NEED_HUMAN', 'APPROVAL_REQUIRED'].includes(humanInput.state) ? humanInput.state : humanRequired ? 'APPROVAL_REQUIRED' : '',
      reason: humanRequired ? (text(humanInput.reason, 500) || needsHumanReason || '高风险任务必须等待人工审批') : '',
    },
  };
}
...
function buildTaskContract({ goal = '', prd = null, source = {}, selectedBy, aiDraft = null, runtimeContext, autoGenerate = false } = {}) {
  ...
  if (autoGenerate) {
    ...
    if (vagueGoal) {
      ...
      needsHumanReason = '当前目标过于笼统，无法安全确定范围和可验证完成条件。';
    }
    if (classification.confidence < 0.7 && !needsHumanReason) needsHumanReason = '任务分析置信度较低，需要人工补充范围和验收条件。';
    const ambiguities = safeList(ai.ambiguities, 8, 500);
    const materialAmbiguities = ambiguities.filter((item) => /(?:权限|授权|审批|删除|清空|覆盖|生产|发布|部署|上线|网络|联网|安装|项目目录之外|跨项目|密钥|token|密码|cookie|.env|私钥|外部副作用|不可逆|git\s*push)/i.test(item));
  ...
  return normalizeTaskContract({
    ...
    humanGate: {
      ...
      state: needsHumanReason ? 'NEED_HUMAN' : classification.riskLevel === 'high' || classification.riskLevel === 'critical' ? 'APPROVAL_REQUIRED' : '',
...
function autoCompleteTaskContract({ contract, fallback = null } = {}) {
  ...
  if (!needsHumanReason && normalized.classification.confidence < 0.7) {
    needsHumanReason = '任务目标信息不足，需要人工补充后才能安全执行。';
  }
  return normalizeTaskContract({
    ...normalized,
    inScope, outOfScope, dod, evidence,
    generationSources, inferredFields, needsHumanReason,
    humanGate: {
      ...normalized.humanGate,
      required: normalized.humanGate.required || Boolean(needsHumanReason),
...
function validateTaskContract({ contract, permissionSnapshot = null } = {}) {
  ...
  if (normalized.classification.confidence < 0.7) reasons.push('任务分析置信度低于自动执行阈值。');
  ...
  const needsHuman = Boolean(normalized.needsHumanReason || normalized.humanGate.state === 'NEED_HUMAN' || normalized.classification.confidence < 0.7);
  const valid = missingFields.length === 0 && !needsHuman && permissionViolations.length === 0 && invalidSources.length === 0;
  ...
  return {
    ...
    suggestedActions: valid ? [] : ['重新分析任务目标', '选择当前项目 PRD 或补充可验证验收条件', ...(permissionViolations.length ? ['检查 AutoPilot 安全策略，或移除未预授权的权限'] : []), ...(needsHuman ? ['进入 NEED_HUMAN，等待人工确认'] : [])],
    needsHuman,
...
async function previewTaskIntake({ projectPath, goal = '', prdMode = 'auto', prdPath = '', allowedRoots = [], env = process.env, fetchImpl = globalThis.fetch, runtimeContext } = {}) {
  ...
  return {
    ...
    analysis: {
      ...
      validation: { valid: validation.valid, missingFields: validation.missingFields, missingFieldLabels: validation.missingFieldLabels, reasons: validation.reasons, suggestedActions: validation.suggestedActions },
    },
  };
}
