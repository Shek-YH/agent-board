...
function createRoutingRuntime({
  ...
  const catalogReader = createCatalogReader({ nativeReader: nativeCapability, cachePath, ttlMs, now, agentVersion });
  ...
  return {
    ...
    async prepare({ workflow, progress = {}, dependencies = null } = {}) {
      ...
      const catalog = await catalogReader.read();
      if (!workflow || String(workflow.agent || '').toLowerCase() !== 'codex') {
