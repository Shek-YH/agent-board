...
function cachePathForAgent(cachePath, agent, { primary = false } = {}) {
...
function createRoutingRuntime({
  ...
  const catalogReaders = new Map([...capabilities.entries()].map(([agent, capability], index) => [agent, createCatalogReader({
    nativeReader: capability,
    cachePath: cachePathForAgent(cachePath, agent, { primary: index === 0 }),
    fallbackReaders: capability && typeof capability.catalogCacheReader === 'function' ? [capability.catalogCacheReader] : [],
    ttlMs, now, agentVersion,
  })]));
  ...
  return {
    supportedAgents() {
      return [...capabilities.entries()]
        .filter(([, capability]) => !capability || capabilityStatus(capability).modelSwitch)
        .map(([agent]) => agent);
    },
    capabilityAgents() {
