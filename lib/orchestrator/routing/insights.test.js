...
test('selects recent task outcomes by completion time rather than input order', () => {
  const done = (id, endedAt, modelId) => ({
    id, agent: 'codex', classification: { kind: 'existing' }, startedAt: endedAt - 10, endedAt,
    runReceipt: {
      finalState: 'DONE', iterations: 1, dod: { passed: 1, total: 1 },
      routing: { modelId, reasoningLevel: 'medium' },
    },
  });

  const result = aggregateHistoricalTaskOutcomes({
    workflows: [done('newest', 300, 'newest-model'), done('oldest', 100, 'oldest-model'), done('middle', 200, 'middle-model')],
    maxRecords: 2,
  });

  assert.deepEqual(result.taskProfiles.map((item) => item.modelId), ['newest-model', 'middle-model']);
});

test('selects recent dispatch outcomes by record time rather than input order', () => {
  const result = aggregateWorkspaceRoutingOutcomes({
    maxRecords: 2,
    workflows: [{ id: 'codex-workflow', agent: 'codex' }],
    dispatchRecords: [
      { workflowId: 'codex-workflow', at: 300, state: 'committed', routing: { modelId: 'newest', reasoningLevel: 'high' } },
      { workflowId: 'codex-workflow', at: 100, state: 'committed', routing: { modelId: 'oldest', reasoningLevel: 'high' } },
      { workflowId: 'codex-workflow', at: 200, state: 'committed', routing: { modelId: 'middle', reasoningLevel: 'high' } },
    ],
  });

  assert.deepEqual(result.profiles.map((item) => item.modelId), ['newest', 'middle']);
});
