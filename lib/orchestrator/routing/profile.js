...
function selectModel(catalog, modelOrder, complexityTier, allowLegacyModels = false) {
  const candidates = [...new Set(Array.isArray(modelOrder) ? modelOrder : [])]
    .map((id) => findModel(catalog, id))
    .filter(Boolean)
    .filter((model) => allowLegacyModels || model.legacy !== true);
...
function nearestReasoning(supported, target, defaultLevel = null) {
  const levels = [...new Set(supported)].map((level) => String(level || '').trim().toLowerCase()).filter(Boolean);
  ...
  if (defaultLevel && levels.includes(defaultLevel)) {
    return {
      ...
      fallbackReason: `requested reasoning ${target} is unsupported; mapped to model default ${defaultLevel}`,
    };
  }
  const targetIndex = REASONING_ORDER.indexOf(target);
  const orderedLevels = levels.filter((level) => REASONING_ORDER.includes(level));
  if (!orderedLevels.length) {
    return {
      level: levels[0],
      fallbackApplied: true,
      fallbackReason: `requested reasoning ${target} is unsupported; mapped to ${levels[0]}`,
    };
  }
  const level = [...orderedLevels].sort((left, right) => {
...
function resolveExecutionProfile({
  ...
  allowLegacyModels = false, requireReasoning = true,
} = {}) {
  ...
  const pinnedModel = manualPin && manualPin.modelId ? findModel(catalog, manualPin.modelId) : null;
  const selected = manualPin && manualPin.modelId
    ? (pinnedModel && (allowLegacyModels || pinnedModel.legacy !== true) ? pinnedModel : null)
    : selectModel(catalog, modelOrder, complexity, allowLegacyModels);
  if (!selected) {
    return {
      modelId: null, reasoningLevel: null, complexityTier: complexity, thinkingTier: thinking,
      promptPolicy, fallbackApplied: false, fallbackReason: null,
      reasonCode: manualPin?.modelId ? 'MANUAL_PIN_UNAVAILABLE' : 'MODEL_UNAVAILABLE',
    };
  }
  ...
  if (!requireReasoning) {
    return {
      ...
      promptPolicy, fallbackApplied: false, fallbackReason: null, reasoningControl: false,
      reasonCode: 'PROFILE_RESOLVED',
      ...(manualPin && manualPin.modelId ? { manualPin: true } : {}),
    };
  }
  ...
  const reasoning = nearestReasoning(getSupportedReasoning(selected), desired, selected.defaultReasoningLevel);
  if (!reasoning.level) {
    return {
      modelId: selected.id, reasoningLevel: null, complexityTier: complexity, thinkingTier: thinking,
