...
test('prefers native catalog and atomically persists a normalized cache', async (t) => {
  ...
  assert.deepEqual(loadCatalogCache(filePath).models[0].supportedReasoningLevels, ['low', 'medium', 'high']);
});
...
test('falls back to the read-only Codex models cache with normalized metadata', async (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-board-codex-cache-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const codexCachePath = path.join(directory, 'models_cache.json');
  fs.writeFileSync(codexCachePath, JSON.stringify({
    fetched_at: '2026-08-31T13:53:49.180955Z',
  ...
  const reader = createCatalogReader({
    cachePath: path.join(directory, 'agent-board-cache.json'),
    codexCachePath,
    now: () => Date.parse('2026-08-31T13:55:00.000Z'),
  ...
  assert.equal(result.available, true);
  assert.equal(result.stale, false);
  assert.equal(result.agentVersion, '0.150.1');
  assert.deepEqual(result.models[0].supportedReasoningLevels, ['low', 'medium', 'high']);
});
