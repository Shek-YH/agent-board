...
test('uses a model-only capability without pretending that reasoning was verified', async () => {
  const result = await applyAndVerifyProfile({
    native: {
      supportsReasoning: false,
      applyModel: async ({ modelId }) => ({ source: 'model-only', readback: { modelId } }),
    },
    sessionRef: 'session-1',
    profile: { modelId: 'strong', reasoningLevel: null, reasoningControl: false },
  });

  assert.equal(result.ok, true);
  assert.equal(result.verified, true);
  assert.equal(result.reasoningControl, false);
  assert.deepEqual(result.readback, { modelId: 'strong' });
});
