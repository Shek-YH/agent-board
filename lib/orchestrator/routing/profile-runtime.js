...
function verifyReadback(profile, readback, { requireReasoning = true } = {}) {
  const expected = expectedProfile(profile);
  const actual = expectedProfile(readback);
  if (!expected.modelId || (requireReasoning && !expected.reasoningLevel)) {
    return { ok: false, code: 'INVALID_PROFILE', reason: 'execution profile is incomplete' };
  }
  if (expected.modelId !== actual.modelId || (requireReasoning && expected.reasoningLevel !== actual.reasoningLevel)) {
    return {
      ok: false,
...
async function applyWithCapability(capability, { sessionRef, profile, modelOnly = false }) {
  const applyModel = capability && typeof capability.applyModel === 'function' ? capability.applyModel.bind(capability) : null;
  const applyProfile = capability && typeof capability.applyProfile === 'function' ? capability.applyProfile.bind(capability) : null;
  if (!capability || (!applyProfile && !(modelOnly && applyModel))) {
    return { ok: false, code: 'PROFILE_APPLY_UNAVAILABLE', dispatchAllowed: false };
  }
  let applied;
  try {
    applied = modelOnly && applyModel
      ? await applyModel({ sessionRef, modelId: profile.modelId, profile })
      : await applyProfile({ sessionRef, profile });
  } catch (error) {
    return {
      ok: false,
  ...
  const verification = verifyReadback(profile, readback, { requireReasoning: !modelOnly });
  ...
  return {
    ...
    ...(modelOnly ? { reasoningControl: false } : {}),
  };
}

async function applyAndVerifyProfile({ native, fallback, allowFallback = false, sessionRef, profile } = {}) {
  const expected = expectedProfile(profile);
  const modelOnly = profile && profile.reasoningControl === false;
  if (!expected.modelId || (!modelOnly && !expected.reasoningLevel)) {
    return { ok: false, code: 'INVALID_PROFILE', dispatchAllowed: false };
  }

  const nativeResult = await applyWithCapability(native, { sessionRef, profile, modelOnly });
  ...
  if (allowFallback && fallback && (typeof fallback.applyProfile === 'function' || (modelOnly && typeof fallback.applyModel === 'function'))) {
    const fallbackResult = await applyWithCapability(fallback, { sessionRef, profile, modelOnly });
