...
function recentItems(items, maxRecords, getTimestamp) {
  const limit = flywheelLimit(maxRecords);
  return (Array.isArray(items) ? items : [])
    .map((item, index) => ({ item, index, timestamp: firstTimestamp(getTimestamp(item)) }))
...
function workflowHistoryTimestamp(workflow) {
  return firstTimestamp(workflow && workflow.endedAt, workflow && workflow.updatedAt, workflow && workflow.createdAt);
}
...
function aggregateWorkspaceRoutingOutcomes({ dispatchRecords = [], workflows = [], events = [], maxRecords = MAX_FLYWHEEL_RECORDS } = {}) {
  ...
  const limit = flywheelLimit(maxRecords);
  const safeRecords = recentItems(dispatchRecords, limit, (record) => record && (record.completedAt ?? record.at ?? record.startedAt))
    .map((record) => {
      if (!record || typeof record !== 'object') return null;
      const agent = workflowAgents.get(text(record.workflowId, 200));
      if (!agent) return null;
      return { state: record.state, routing: record.routing, agent, at: record.at };
    })
    .filter(Boolean);
...
function aggregateHistoricalTaskOutcomes({ workflows = [], events = [], maxRecords = MAX_FLYWHEEL_RECORDS } = {}) {
  ...
  for (const workflow of recentItems(workflows, maxRecords, workflowHistoryTimestamp)) {
