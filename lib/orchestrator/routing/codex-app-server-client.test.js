...
test('performs the app-server handshake and request/response exchange', async () => {
  ...
  assert.deepEqual(child.requests[0].params.capabilities, { experimentalApi: true });
  await client.close();
});
...
test('supports an unbounded notification wait for long-running turns', async () => {
  const child = new FakeChild();
  const client = createCodexAppServerClient({ executablePath: 'codex.exe', spawnImpl: () => child });
  await client.request('model/list', {});
  const waiting = client.waitForNotification({ method: 'turn/completed', timeoutMs: 0 });
  await new Promise((resolve) => setImmediate(resolve));
  child.emitJson({ method: 'turn/completed', params: { threadId: 'thread-1', turn: { id: 'turn-1', status: 'completed' } } });
...
test('waits for the app-server process to exit before close resolves', async () => {
  const child = new FakeChild();
  child.kill = () => {};
  const client = createCodexAppServerClient({ executablePath: 'codex.exe', spawnImpl: () => child });
  await client.request('model/list', {});
...
test('Windows close terminates the complete app-server process tree', async () => {
  const child = new FakeChild();
  child.pid = 4321;
  const client = createCodexAppServerClient({
  ...
  client.execFileImpl = (_command, args, _options, callback) => {
    killedTreePid = Number(args[1]);
    child.emit('exit', 0, null);
    callback(null);
  };
