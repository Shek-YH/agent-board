...
const REASONING_LEVELS = new Set(['none', 'low', 'medium', 'high', 'xhigh', 'max', 'ultra']);
...
function normalizeReasoning(value) {
  ...
  return text(value.effort || value.level || value.reasoning_level || value.reasoningLevel
    || value.reasoning_effort || value.reasoningEffort || value.name || value.id).toLowerCase();
}
...
function reasoningValues(model) {
  const fields = [
    ...
    'reasoning_levels', 'reasoningLevels', 'reasoning_efforts', 'reasoningEfforts',
  ];
  for (const field of fields) {
    if (Array.isArray(model[field])) return model[field];
  }
  return [];
}
...
function normalizeModel(model) {
  ...
  const id = text(model.slug || model.id || model.model_id || model.model);
  if (!id) return null;
  const visibilityValue = text(model.visibility).toLowerCase();
  const hidden = model.hidden === true || model.is_hidden === true || ['hide', 'hidden'].includes(visibilityValue);
  const legacy = model.legacy === true || model.isLegacy === true || model.is_legacy === true
    || text(model.modelType || model.model_type).toLowerCase() === 'legacy';
  const supportedReasoningLevels = [...new Set(
    reasoningValues(model)
      .map(normalizeReasoning)
      .filter(Boolean),
  )];
  const defaultReasoningLevel = normalizeReasoning(
    model.default_reasoning_level || model.default_reasoning_effort || model.defaultReasoningLevel || model.defaultReasoningEffort,
  );
  const normalized = {
    id,
    ...
    visibility: hidden ? 'hidden' : (visibilityValue || 'list'),
  ...
  const version = text(model.version || model.modelVersion || model.model_version);
  const multiAgentVersion = text(model.multiAgentVersion || model.multi_agent_version);
  if (version) normalized.version = version;
  if (multiAgentVersion) normalized.multiAgentVersion = multiAgentVersion;
