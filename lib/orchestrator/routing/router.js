...
function resolveRoutingDecision({
  ...
  const signal = pressure({ consecutiveFailures, regression, stagnationCount, lastRunSucceeded, taskSimplified });
  const request = {
  ...
  const resolvedProfile = resolveExecutionProfile({
    catalog, modelOrder, complexityTier: request.complexityTier, thinkingTier: request.thinkingTier,
    promptPolicy: request.promptPolicy, manualPin: request.manualPin,
  });
  ...
  const reasonCode = resolvedProfile.manualPin ? 'MANUAL_PIN' : signal.reasonCode;
  return { enabled: true, routeRequest: request, resolvedProfile, action: 'apply', reasonCode, catalog: requestCatalog(catalog) };
}
