...
test('starts a fresh route after a completed subtask instead of reusing the previous route trigger', () => {
  const decision = resolveRoutingDecision({
    enabled: true, catalog: catalog(), modelOrder: ['strong', 'balanced', 'fast'],
    complexityTier: 'C1', thinkingTier: 'T1', lastRouting: { modelId: 'fast', reasoningLevel: 'low' },
    freshRoute: true,
  });

  assert.equal(decision.action, 'apply');
  assert.equal(decision.routeRequest.trigger, 'ROUTE_FRESH');
  assert.equal(decision.reasonCode, 'ROUTE_FRESH');
});

test('respects disabled automatic model and reasoning changes using the accepted route', () => {
  const decision = resolveRoutingDecision({
    enabled: true, catalog: catalog(), modelOrder: ['strong', 'balanced', 'fast'],
    complexityTier: 'C0', thinkingTier: 'T3', lastRouting: { modelId: 'balanced', reasoningLevel: 'high' },
    autoModel: false, autoReasoning: false,
  });

  assert.equal(decision.action, 'apply');
  assert.equal(decision.resolvedProfile.modelId, 'balanced');
  assert.equal(decision.resolvedProfile.reasoningLevel, 'high');
  assert.equal(decision.reasonCode, 'ROUTE_STABLE');
});
