...
test('exposes all registered capability agents for transparent capability reporting', () => {
  ...
  assert.deepEqual(runtime.supportedAgents(), ['codex']);
});
...
test('keeps per-agent catalog caches isolated when adapters share a base path', () => {
  assert.equal(cachePathForAgent('C:\\tmp\\routing.json', 'codex', { primary: true }), 'C:\\tmp\\routing.json');
...
test('isolates non-Codex adapters when the caller uses the default cache path', () => {
  const codexPath = cachePathForAgent(undefined, 'codex', { primary: true });
...
test('uses adapter-owned catalog fallback without agent-specific routing branches', async (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-board-routing-adapter-cache-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const runtime = createRoutingRuntime({
    cachePath: path.join(directory, 'routing.json'),
    nativeCapability: null,
    agentCapabilities: {
      secondary: {
        listModels: async () => { throw new Error('native catalog unavailable'); },
        catalogCacheReader: async () => ({
          source: 'adapter-cache', fetchedAt: 2_000, agentVersion: 'secondary-1',
          models: [{ id: 'secondary-strong', displayName: 'Secondary Strong', supportedReasoningLevels: ['high'] }],
        }),
      },
    },
    agentVersion: 'secondary-1',
    now: () => 2_000,
  });

  const result = await runtime.getCatalog({ agent: 'secondary' });

  assert.equal(result.source, 'adapter-cache');
  assert.equal(result.available, true);
  assert.equal(result.models[0].id, 'secondary-strong');
});

test('supports an explicit primary cache slot without agent-specific routing branches', () => {
  assert.equal(cachePathForAgent('C:\\tmp\\routing.json', 'primary', { primary: true }), 'C:\\tmp\\routing.json');
  assert.equal(cachePathForAgent('C:\\tmp\\routing.json', 'secondary'), 'C:\\tmp\\routing-secondary.json');
});
