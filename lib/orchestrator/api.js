...
function createWorkflowRequest({
  ...
  const runContract = normalizeRunContract({
    ...
    ...(handoffChain === undefined ? {} : { handoffChain }),
  });
...
async function previewSessionTaskIntake({
  ...
  let preview;
  try {
    preview = await previewTaskIntake({ projectPath, goal: requestedGoal, prdMode, prdPath, allowedRoots, env, fetchImpl, researcher,
      permissionSnapshot: settings ? buildPermissionSnapshot({ projectPath, settings }) : null, runtimeContext });
  } catch (error) {
    ...
    preview = await previewTaskIntake({ projectPath, goal: fallbackGoal, prdMode, prdPath, allowedRoots, env, fetchImpl, researcher,
      permissionSnapshot: settings ? buildPermissionSnapshot({ projectPath, settings }) : null, runtimeContext });
  }
  return {
    ...preview,
...
function taskContractValidationError(validation) {
  ...
  error.researchRequired = validation.researchRequired;
  return error;
}
...
async function previewProjectTaskIntake({
  agent, projectPath, goal, prdMode, prdPath, sessionStore, allowedRoots = [], env = process.env, fetchImpl = globalThis.fetch, settings, researcher = null,
} = {}) {
  ...
  const preview = await previewTaskIntake({
    ...
    researcher, permissionSnapshot: settings ? buildPermissionSnapshot({ projectPath: context.projectPath, settings }) : null,
...
async function createConfirmedTask({
  ...
  if (!validation.valid && !validation.needsHuman && validation.researchRequired) {
    throw taskContractValidationError(validation);
  }
...
async function createSessionIntakeWorkflowRequest({
  ...
  const preview = await previewSessionTaskIntake({
    sessionRef, goal, prdMode, prdPath, sessionStore, allowedRoots, env, fetchImpl, researcher,
    settings,
  });
  ...
  if (!validation.valid && !validation.needsHuman && validation.researchRequired) throw taskContractValidationError(validation);
  contract = applyValidationHumanGate(contract, validation);
  const verify = {
    dod: contract.dod.length ? contract.dod : ['任务目标已完成并通过验证'],
    evidence: contract.evidence.length ? contract.evidence : ['Agent Board verified execution'],
