...
function safeProof(proof, kind = 'generic') {
  ...
  const proofId = safeText(proof.messageId || proof.deliveryId || proof.eventId, 200);
...
function normalizeLoadedWorkflow(workflow) {
  ...
  if (normalized.settingsSnapshot) {
    try { normalized.settingsSnapshot = normalizeSettings(normalized.settingsSnapshot); } catch { normalized.settingsSnapshot = null; }
  } else {
...
class WorkflowStore {
  ...
  create({
    ...
    const workflow = {
      ...
      settingsSnapshot: settingsSnapshot ? deepClone(settingsSnapshot) : null,
      binding: binding ? {
