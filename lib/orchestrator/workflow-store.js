...
function normalizeDispatchRecord(record) {
  ...
  return {
    ...
    routing: normalizeRoutingSummary(record.routing), sessionRef: safeText(record.sessionRef, 200), at: Number.isFinite(record.at) ? record.at : 0,
  };
}
...
class WorkflowStore {
  ...
  appendDispatchRecord(workflowId, record = {}) {
    ...
    const safe = {
      ...
      sessionRef: safeText(record.sessionRef, 200), at: Date.now(),
    };
