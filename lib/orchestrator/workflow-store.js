...
class WorkflowStore {
  ...
  load() {
    try {
      ...
      if (raw && Array.isArray(raw.workflows) && Array.isArray(raw.events)) this.data = raw;
    } catch (error) {
  ...
  create({ projectPath, mode = 'project', agent = '', classification = null, executionPlan = null, requestedBy = 'human' }) {
    const workflow = {
