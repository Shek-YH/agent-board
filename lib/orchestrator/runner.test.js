...
test('runner executes project lifecycle then completes a successful worker run', async () => {
  ...
  const created = createWorkflowRequest({ projectPath: path.join(dir, 'new-app'), goal: '创建项目', store });
...
test('runner pauses when project lifecycle needs human approval', async () => {
  ...
  const created = createWorkflowRequest({ projectPath: project, goal: '维护项目', store });
  const runner = new WorkflowRunner({
...
test('runner records worker failure and releases the project lease', async () => {
  const { dir, store } = setup();
  const created = createWorkflowRequest({ projectPath: path.join(dir, 'new-app'), goal: '失败任务', store });
  const runner = new WorkflowRunner({
...
test('runner cannot resume a workflow held by human takeover', async () => {
  const { dir, store } = setup();
  const created = createWorkflowRequest({ projectPath: path.join(dir, 'new-app'), goal: 'x', store });
