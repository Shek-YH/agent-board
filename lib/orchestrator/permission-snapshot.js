...
function researchDomains(value) {
  return list(value, [], 100, 253).map((item) => item.toLowerCase().replace(/^\*\./, ''))
    .filter((item) => /^[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?(?::\d+)?$/.test(item));
}
...
function payloadFor(snapshot) {
  return {
    ...
    researchRead: snapshot.researchRead,
    allowedResearchDomains: snapshot.allowedResearchDomains,
...
function normalizePermissionSnapshot(input = {}) {
  ...
  const normalized = {
    ...
    researchRead: bool(source.researchRead),
    allowedResearchDomains: researchDomains(source.allowedResearchDomains),
...
function buildPermissionSnapshot({ projectPath, settings = {}, now = Date.now() } = {}) {
  ...
  return normalizePermissionSnapshot({
    ...
    researchRead: safety.researchRead === true,
    allowedResearchDomains: safety.allowedResearchDomains,
