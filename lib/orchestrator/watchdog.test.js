...
test('watchdog uses per-run failure, stagnation, and max budget limits', () => {
  const configured = workflow({ runContract: { budget: {
    maxIterations: 3, maxRuntime: 60_000, maxBudget: 10, failureThreshold: 4, stagnationThreshold: 3, supervisorCostLimit: 0,
  } } });
  assert.equal(checkWatchdog({ workflow: { ...configured, consecutiveFailures: 4 }, instruction: 'x', now: 2_000 }).reason, 'Blocked');
  assert.equal(checkWatchdog({ workflow: { ...configured, stagnationCount: 3 }, instruction: 'x', now: 2_000 }).reason, 'Stagnation');
  assert.equal(checkWatchdog({ workflow: { ...configured, supervisorCostUsed: 10 }, instruction: 'x', now: 2_000 }).reason, 'Budget Exceeded');
});
