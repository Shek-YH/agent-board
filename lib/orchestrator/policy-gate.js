...
function evaluatePolicyGate({ workflow, capabilities = {}, session, instruction = '', watchdog, allowedRoots = [] } = {}) {
  ...
  const taskContract = workflow.taskContract && typeof workflow.taskContract === 'object' ? workflow.taskContract : null;
  const researchState = workflow.researchState && typeof workflow.researchState === 'object' ? workflow.researchState : null;
  const researchPending = taskContract && (['research_required', 'researching'].includes(taskContract.generationStatus)
    || ['required', 'researching', 'unresolved', 'permission_required', 'conflict', 'timeout', 'budget_exceeded'].includes(researchState && researchState.status));
  checks.push(researchPending
    ? fail('research', 'Task Contract research is not complete', 'RESEARCH_REQUIRED') : pass('research'));
  ...
  const operationalInstruction = instructionText
    .replace(/<ai_hosted_agent_contract>[\s\S]*?<\/ai_hosted_agent_contract>/gi, '')
    .split(/\r?\n/)
    .filter((line) => !/^\s*(?:范围外：|不要执行范围外)/.test(line))
    .join('\n');
  // 剥离「否定式安全禁令」：指令中的「不删除 / 不要 git push / 禁止安装依赖 / 不引入网络请求」
  // 是合同、范围外/范围内提示，不是实际的越权请求。剥离后再做危险词匹配，避免把「禁止做X」
  // 误判成「要做X」（如「不引入外部依赖、网络请求或权限提升操作」被误读为联网越权）而阻断托管派发。
  const operational = operationalInstruction
    .replace(/(?:不|不要|禁止|不得|不会|不应|无需|暂不|未|避免|绝不)\s*(?:执行|进行|使用|读取|输出|发送|获取|暴露|泄露|安装|联网|网络请求|网络访问|访问|修改|删除|清空|覆盖|部署|发布|引入|git\s+push|越(?:过|权)|升级权限|绕过审批|自动授权|超出)[^\n。；]*/gi, '')
    .replace(/\b(?:do\s*not|never|without|avoid)\b[^\n]*/gi, '');
  const dangerous = DANGEROUS_PATTERNS.some((pattern) => pattern.test(operational));
  const secretAccess = /(?:\.env(?:\.|\b)|(?:api[_ -]?key|token|password|private[_ -]?key))/i.test(operational);
  const networkAccess = /(?:curl|wget|invoke-webrequest|联网|网络请求|http[s]?:\/\/)/i.test(operational);
  const installAccess = /(?:npm\s+(?:install|i)|pnpm\s+(?:install|i)|yarn\s+add|pip\s+install|cargo\s+add|安装依赖)/i.test(operational);
  const gitPush = /\bgit\s+push\b/i.test(operational);
  const allowedDomains = Array.isArray(snapshot.allowedNetworkDomains) ? snapshot.allowedNetworkDomains : [];
  const hosts = networkHosts(operational);
