...
function evaluatePolicyGate({ workflow, capabilities = {}, session, instruction = '', watchdog, allowedRoots = [] } = {}) {
  ...
  checks.push(sameProject && inAllowedRoot
    ? pass('scope')
    : fail('scope', 'session binding must remain inside the workflow project scope', 'SCOPE_INVALID'));
  ...
  const dangerous = DANGEROUS_PATTERNS.some((pattern) => pattern.test(String(instruction || '')));
  checks.push(dangerous ? fail('danger', 'dangerous instruction requires human confirmation', 'PERMISSION_REQUIRED') : pass('danger'));
