...
function evaluatePolicyGate({ workflow, capabilities = {}, session, instruction = '', watchdog, allowedRoots = [] } = {}) {
  ...
  const dangerous = DANGEROUS_PATTERNS.some((pattern) => pattern.test(instructionText));
  const secretAccess = /(?:\.env(?:\.|\b)|(?:api[_ -]?key|token|password|private[_ -]?key))/i.test(instructionText);
  const networkAccess = /(?:curl|wget|invoke-webrequest|联网|网络请求|http[s]?:\/\/)/i.test(instructionText);
  const installAccess = /(?:npm\s+(?:install|i)|pnpm\s+(?:install|i)|yarn\s+add|pip\s+install|cargo\s+add|安装依赖)/i.test(instructionText);
  const gitPush = /\bgit\s+push\b/i.test(instructionText);
  const allowedDomains = Array.isArray(snapshot.allowedNetworkDomains) ? snapshot.allowedNetworkDomains : [];
  const hosts = networkHosts(instructionText);
