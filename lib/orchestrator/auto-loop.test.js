...
function setup({ dispatchResult, routingRuntime = null, verifyDeliveryFingerprint = null, reconcileEveryTurns, dod = ['测试通过'], completionDetector, bindingTitle = '', bindingTransport = '', sessionMessages = [], supervisorReview = null, evidenceCollector = null } = {}) {
  ...
  const workflow = store.create({
    ...
    runContract: normalizeRunContract({ autopilotMode: 'auto', goal: '完成目标', verify: { dod } }),
  });
  let sends = 0;
  ...
  const dependencies = {
    resolveSession: async (request) => {
      ...
      return {
      status: 'resolved',
      target: { sessionRef: 'session-1', agent: 'codex', project: projectPath, role: 'main', controlEligibility: 'eligible', messages: sessionMessages },
  ...
  const loop = new AutoLoop({
    ...
    routingRuntime, reconcileEveryTurns,
    ...
    }, now: () => 2_000, supervisorReview, evidenceCollector,
  });
