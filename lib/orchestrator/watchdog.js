...
function checkWatchdog({ workflow, instruction = '', now = Date.now() } = {}) {
  ...
  const checks = [
    ...
    { name: 'budget', allowed: !Number(budget.maxBudget) || Number(workflow.supervisorCostUsed || 0) < Number(budget.maxBudget), reason: 'Budget Exceeded' },
    ...
    { name: 'failures', allowed: Number(workflow.consecutiveFailures || 0) < Number(budget.failureThreshold || MAX_FAILURES), reason: 'Blocked' },
    { name: 'dispatch_failures', allowed: Number(workflow.dispatchFailures || 0) < Number(budget.failureThreshold || MAX_FAILURES), reason: 'Blocked' },
    { name: 'stagnation', allowed: Number(workflow.stagnationCount || 0) < Number(budget.stagnationThreshold || MAX_STAGNATION), reason: 'Stagnation' },
