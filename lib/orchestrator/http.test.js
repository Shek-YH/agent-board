...
function setup({ routingNativeCapability = null, researcher = null } = {}) {
  ...
  const runtime = createOrchestrationRuntime({
    ...
    routingNativeCapability, researcher,
  });
  return { dir, runtime };
}
...
test('HTTP workflow API keeps hosted task relation metadata bounded and visible', async () => {
  const { dir, runtime } = setup();
  const created = await handleOrchestrationRequest({
    method: 'POST', pathname: '/api/orchestration/workflows', runtime,
    body: {
      projectPath: path.join(dir, 'hosted-app'), goal: '托管任务', agent: 'codex', autopilotMode: 'auto',
      verify: { dod: ['完成'] },
      hostedControl: { enabled: true, sourceTaskId: 'source-1', targetTaskId: 'target-1', hostId: 'host-1', lastAgentMessageText: 'secret raw reply' },
    },
  });
  assert.equal(created.status, 201);
  assert.equal(created.body.workflow.hostedControl.targetTaskId, 'target-1');
  assert.doesNotMatch(JSON.stringify(created.body), /secret raw reply|lastAgentMessageText/);
});

test('HTTP workflow API accepts and exposes a normalized handoffChain', async () => {
  const { dir, runtime } = setup();
  const created = await handleOrchestrationRequest({
    method: 'POST', pathname: '/api/orchestration/workflows', runtime,
    body: {
      projectPath: path.join(dir, 'chain-app'), goal: '接力执行', agent: 'codex', autopilotMode: 'auto',
      verify: { dod: ['完成'] }, handoffChain: [
  ...
  assert.equal(created.status, 201);
  assert.deepEqual(created.body.workflow.handoffChain.map((step) => step.id), ['build', 'review']);
  assert.equal(created.body.workflow.handoffChain[1].status, 'pending');
});

test('HTTP research status is available by draft id without creating a workflow', async () => {
  const researcher = {
    research: async () => ({
      status: 'completed', confidence: 0.9, confidenceLevel: 'high', sources: [], findings: {}, unresolvedQuestions: [],
    }),
  };
  const { dir, runtime } = setup({ researcher });
  const preview = await handleOrchestrationRequest({
    method: 'POST', pathname: '/api/orchestration/intake/preview', runtime,
    body: { agent: 'codex', projectPath: dir, goal: '完成目标但不说明要改什么或如何验收。', prdMode: 'none' },
  });
  assert.equal(preview.status, 200);
  assert.equal(runtime.store.list().length, 0);
  const research = await handleOrchestrationRequest({
    method: 'GET', pathname: `/api/orchestration/intake/research/${preview.body.draftId}`, runtime,
  });
  assert.equal(research.status, 200);
  assert.equal(research.body.research.status, 'completed');
});

test('HTTP intake passes the immutable research permission snapshot to the injected researcher', async () => {
  let snapshot = null;
  const researcher = {
    research: async (request) => {
      snapshot = request.permissionSnapshot;
      return { status: 'completed', confidence: 0.9, sources: [], findings: {}, unresolvedQuestions: [] };
    },
  };
  const { dir, runtime } = setup({ researcher });
  const preview = await handleOrchestrationRequest({
    method: 'POST', pathname: '/api/orchestration/intake/preview', runtime,
    body: {
      agent: 'codex', projectPath: dir, goal: '完成目标但不说明要改什么或如何验收。', prdMode: 'none',
      settings: { safety: { researchRead: true, allowedResearchDomains: ['docs.example.com'] } },
    },
  });
  assert.equal(preview.status, 200);
