...
test('accepts the conventional lower-case model key from a local env file', () => {
  const state = getProviderConfigState({ env: { ZHIPU_API_KEY: 'test-only-key', model: 'glm-local' } });
  assert.equal(state.supervisor.provider, 'zai');
  assert.equal(state.supervisor.model, 'glm-local');
  assert.equal(state.supervisor.available, true);
});
