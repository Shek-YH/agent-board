...
function saveTaskDraft(runtime, result, hostedControl) {
  ...
  runtime.taskDrafts.set(id, {
    ...
    ...(hostedControl !== undefined ? { hostedControl: normalizeHostedControl(hostedControl) } : {}),
  });
  return id;
}
...
async function handleOrchestrationRequest({ method, pathname, query, body = {}, runtime }) {
  ...
  if (pathname === '/api/orchestration/workflows' && verb === 'POST') {
    const result = createWorkflowRequest({
      ...
      handoffChain: body.handoffChain,
      hostedControl: body.hostedControl,
  ...
  if (pathname === '/api/orchestration/intake/preview' && verb === 'POST') {
    try {
      if (body.sessionRef) {
        ...
        const draftId = saveTaskDraft(runtime, result, body.hostedControl);
  ...
  const researchDraftId = routeMatch(pathname, /^\/api\/orchestration\/intake\/research\/([^/]+)$/);
  if (researchDraftId && verb === 'GET') {
    const draft = readTaskDraft(runtime, decodeURIComponent(researchDraftId));
    if (!draft) return { status: 404, body: { ok: false, code: 'TASK_DRAFT_NOT_FOUND', error: 'Task Contract 草稿不存在或已过期' } };
    return {
      status: 200,
      body: {
        ok: true, draftId: draft.id, research: draft.taskContract && draft.taskContract.research || null,
  ...
  if (pathname === '/api/orchestration/intake/confirm' && verb === 'POST') {
    ...
    try {
      const result = await createConfirmedTask({
        ...
        hostedControl: body.hostedControl === undefined ? draft.hostedControl : body.hostedControl,
      });
      draft.consumed = true;
      ...
      if (error && error.code) {
        return {
          ...
          body: {
            ...
            ...(Array.isArray(error.missingFieldLabels) ? { missingFieldLabels: error.missingFieldLabels } : {}),
            ...(Array.isArray(error.reasons) ? { reasons: error.reasons } : {}),
            ...(Array.isArray(error.suggestedActions) ? { suggestedActions: error.suggestedActions } : {}),
            ...(Array.isArray(error.permissionViolations) ? { permissionViolations: error.permissionViolations } : {}),
            ...(typeof error.researchRequired === 'boolean' ? { researchRequired: error.researchRequired } : {}),
          },
        };
      }
      throw error;
