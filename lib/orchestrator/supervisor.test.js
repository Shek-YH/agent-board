...
test('auto supervisor instructions identify the Agent as a hosted multi-turn worker', () => {
  const result = buildSupervisorDecision({
    workflow: workflow({ autopilotMode: 'auto' }),
    progress: { status: 'in_progress', completed: 0, total: 2, evidence: [], stopReason: null },
  });

  assert.match(result.instruction, /<ai_hosted_agent_contract>/);
  assert.match(result.instruction, /STATUS:/);
  assert.match(result.instruction, /不要.*等待用户|不要.*请确认/);
});
...
test('AI Supervisor does not turn an ordinary hosted Agent question into NEED_HUMAN', () => {
  const current = workflow({ autopilotMode: 'auto' });
  const decision = buildSupervisorDecision({
    workflow: current, progress: { status: 'in_progress', evidence: [], stopReason: null },
  });
  const reviewed = applySupervisorReview({
    workflow: current, progress: { status: 'in_progress', evidence: [], stopReason: null }, decision,
    hostedReply: { status: 'question', requiresHostDecision: true, safetyBoundary: false },
    reviewResult: { source: 'model', provider: 'zai', model: 'glm-test', review: { decision: 'NEED_HUMAN', summary: '普通布局问题', dodChecks: [] } },
  });
  assert.equal(reviewed.decision, 'CONTINUE');
  assert.equal(reviewed.reasonCode, 'HOSTED_AGENT_QUESTION');
});

test('AI Supervisor preserves NEED_HUMAN for WAITING_FOR_HOST', () => {
  const current = workflow({ autopilotMode: 'auto' });
  const decision = buildSupervisorDecision({
    workflow: current, progress: { status: 'in_progress', evidence: [], stopReason: null },
  });
  const reviewed = applySupervisorReview({
    workflow: current, progress: { status: 'in_progress', evidence: [], stopReason: null }, decision,
    hostedReply: { status: 'waiting_for_host', requiresHostDecision: true, safetyBoundary: false },
    reviewResult: { source: 'model', provider: 'zai', model: 'glm-test', review: { decision: 'NEED_HUMAN', summary: '需要人工确认', dodChecks: [] } },
  });
  assert.equal(reviewed.decision, 'NEED_HUMAN');
});
