...
class AutoLoop {
  ...
  async runOnce(id) {
    ...
    const sessionSnapshot = await this.readSessionSnapshot(workflow, dependencies, session);
    ...
    try {
      result = await this.guiBus.run({ agent: workflow.agent, sessionRef: workflow.binding.sessionRef }, async () => {
