...
function isNativeFirstTurn(workflow, dependencies) {
  return Number(workflow && workflow.runCount || 0) === 0
...
class AutoLoop {
  ...
  async runOnce(id) {
    ...
    const nativeFirstTurn = isNativeFirstTurn(workflow, dependencies);
    const session = nativeFirstTurn ? nativeSessionTarget(workflow) : await this.preflightSession(workflow, dependencies);
    const sessionSnapshot = nativeFirstTurn ? { messages: [] } : await this.readSessionSnapshot(workflow, dependencies, session);
    ...
    if (typeof this.supervisorReview === 'function') {
      try {
        reviewResult = await this.supervisorReview({ workflow, progress, session, decision });
      } catch (_) {
        reviewResult = null;
      }
    }
    decision = applySupervisorReview({ workflow, progress, decision, reviewResult });
    ...
    const deliverySnapshot = nativeFirstTurn ? null : await this.captureDeliveryBoundary(session, dependencies);
    if (!nativeFirstTurn && dependencies && typeof dependencies.captureDeliverySnapshot === 'function' && !deliverySnapshot) {
      workflow = this.store.updateFields(id, {
    ...
    workflow = this.store.get(id) || workflow;
