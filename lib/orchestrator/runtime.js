...
function createOrchestrationRuntime({
  env = process.env, filePath = defaultWorkflowPath(), settingsPath, sessionStore, runCommand, onWorkflowChange, workbuddyCliPath, nodeExecutable,
  verifiedDispatchDependencies, autoDispatch, autoNow, routingNativeCapability, routingProfileFallback,
  routingAgentCapabilities, routingCachePath, routingAgentVersion, routingProbeObserver, reconcileEveryTurns,
} = {}) {
  ...
  return { store, settings, sessionStore, runner, auto, routing, suggestion, transport, allowedRoots, headlessEnabled: transport.enabled, env, notify };
}
