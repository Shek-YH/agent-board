...
function createOrchestrationRuntime({
  ...
  verifiedDispatchDependencies, autoDispatch, autoNow,
} = {}) {
  ...
  const auto = new AutoLoop({
    ...
    dispatch: autoDispatch, now: autoNow || (() => Date.now()), onWorkflowChange: notify,
  });
  auto.reconcileAll().catch(() => {});
  auto.startPeriodicReconciliation(Number(env.AGENT_BOARD_AUTOPILOT_RECONCILE_MS) || 60_000);
  return { store, runner, auto, suggestion, transport, allowedRoots, headlessEnabled: transport.enabled, env, notify };
}
