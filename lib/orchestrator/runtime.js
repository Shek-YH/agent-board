...
function createOrchestrationRuntime({ env = process.env, filePath = defaultWorkflowPath(), runCommand, onWorkflowChange } = {}) {
  const store = new WorkflowStore(filePath);
  const allowedRoots = parseAllowedRoots(env.AGENT_BOARD_ALLOWED_ROOTS);
  const transport = new HeadlessTransport({ enabled: env.AGENT_BOARD_HEADLESS_EXECUTION === '1' });
  ...
  return { store, runner, transport, allowedRoots, headlessEnabled: transport.enabled, env, notify };
}
